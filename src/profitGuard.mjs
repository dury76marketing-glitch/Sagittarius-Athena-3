...
function stopLossForEntry(entry, settings) {
  if (n(entry.stopLossCents) > 0) return n(entry.stopLossCents);
  if (entry.conceptName === 'Momentum Hunter') return n(settings.momentumHunterStopLossCents, 10);
  if (entry.conceptName === 'Recovery Hunter') return n(settings.recoveryHunterStopLossCents, 10);
  if (entry.conceptName === 'Crash Recovery Hunter') return n(settings.crashRecoveryStopLossCents, 35);
  if (entry.conceptName === 'Dragon Recovery Hunter') return n(settings.dragonRecoveryStopLossCents, 35);
  if (entry.conceptName === 'Golden Dragon Hunter') return n(settings.goldenDragonHunterStopLossCents, 35);
  return n(settings.waveStopCents, 14);
}
...
export function profitGuardDecision(entry, q, settings) {
  ...
  const stopLoss = entry.stopLossCents > 0
    ? entry.stopLossCents
    : entry.conceptName === 'Momentum Hunter'
      ? (settings.momentumHunterStopLossCents ?? 10)
      : entry.conceptName === 'Recovery Hunter'
        ? (settings.recoveryHunterStopLossCents ?? 10)
        : entry.conceptName === 'Crash Recovery Hunter'
          ? (settings.crashRecoveryStopLossCents ?? 35)
          : entry.conceptName === 'Dragon Recovery Hunter'
            ? (settings.dragonRecoveryStopLossCents ?? 35)
            : entry.conceptName === 'Golden Dragon Hunter'
              ? (settings.goldenDragonHunterStopLossCents ?? 35)
              : (settings.waveStopCents ?? 14);
...
export class ProfitGuard {
  constructor({ db, kalshi, market, learning, athena = null, getSettings }) {
    this.db = db;
    this.kalshi = kalshi;
    this.market = market;
  ...
  fullPositionCounterfactualNetCents(entry, priceCents, settings, { settlement = false } = {}) {
    ...
    const exitFee = settlement ? 0 : n(settings.simFeeCents) * qty;
  ...
  async migrateLegacyUltimateProfitGuardState(entry, q, state, count, settings, observedBookMs = Date.now()) {
    ...
    if (headroomArmPriceCents > 100 || peakExecutableBidCents < headroomArmPriceCents) {
      const hardStopCents = Math.max(0, n(entry.entryPriceCents) - stopLossForEntry(entry, settings));
      await this.db.updateEntry(entry.id, {
        profitGuardState:{},
  ...
  async reconcilePending(entry) {
    ...
    if (truth?.fillCount > 0) {
      ...
      const patch = {
        ...
        entryOrderId: truth.orderId,
        count: truth.fillCount,
        remainingCount: truth.fillCount,
        entryFeeCents: fee,
        currentPriceCents: price,
        peakPriceCents: price,
        updatedAtMs: Date.now(),
      };
      await this.db.updateEntry(entry.id, patch);
      await this.audit('entry_pending_reconciled_fill', { id: entry.id, ticker: entry.ticker, clientOrderId: entry.entryClientOrderId, fillCount: truth.fillCount, priceCents: price });
  ...
  async protectTicker(ticker) {
    const s = this.getSettings();
    const entries = await this.db.openEntriesByTicker(s.systemName, ticker);
  ...
  async sweep() {
    try {
      ...
      this.lastError = null;
      const s = this.getSettings();
      const entries = await this.db.openEntries(s.systemName);
  ...
  async trackPostExit() {
    const s = this.getSettings();
    const closed = await this.db.recentClosed(s.systemName, 500);
    const now = Date.now();
    const displayCut = now - 4 * 3600000;
    ...
    for (const e of closed) {
      ...
      // Profit learning is asynchronous by design. Post-exit research may wait
      // briefly for the ordered per-entry queue, but it never blocks the live
      // protection path. If learning storage is unhealthy, skip this research
      // pass and retry later rather than observing events out of order.
      if (profitLearningTrade) {
      ...
      const needsDisplay = e.closedAtMs >= displayCut;

      // Legacy non-hard-stop exits preserve the pre-R32 lifecycle. R32 Hunters
      // are intentionally excluded: PLI1 owns their independent post-exit
      // learning lifecycle even when the old sag_entries research flag changes.
      ...
      if (!needsPliResearch && !needsRecoveryResearch && !needsDisplay) {
        ...
        if (profitLearningTrade && !pliComplete && researchExpired && typeof this.learning?.finalizeProfitLearning === 'function') {
          try {
            ...
            await this.audit('pli1_post_exit_finalize_failed', { id:e.id, ticker:e.ticker, error:String(error?.message || error) }, 'warning');
          }
        }
        continue;
      }
      ...
      if (!q || this.market.quoteAgeMs(e.ticker) > 30_000) q = await this.market.refreshTicker(e.ticker).catch(() => q);
      if (!q) {
      ...
      const isFinal = finalMarket(q);
      const finalPayout = isFinal ? (String(q.result).toLowerCase() === 'yes' ? 100 : 0) : null;
      const displayPrice = isFinal
        ? finalPayout
        : (q.yesBid > 0 && q.yesAsk > 0 ? Math.round((q.yesBid + q.yesAsk) / 2) : (q.yesBid || q.yesAsk || e.currentPriceCents));
      const patch = {};
      if (needsDisplay) patch.currentPriceCents = displayPrice;
      ...
      if (needsPliResearch) {
        try {
          if (isFinal) {
            const terminalNetCents = this.fullPositionCounterfactualNetCents(e, finalPayout, s, { settlement:true });
            if (typeof this.learning?.observeProfitPostExit === 'function') {
              await this.learning.observeProfitPostExit(e, {
                executableBidCents:finalPayout, executableNetCents:terminalNetCents,
                observedAtMs:Math.max(n(q?.updatedAtMs, now), n(e.closedAtMs)),
                finalResult:String(q.result || '').toLowerCase(), terminalNetCents,
              });
            }
            if (typeof this.learning?.finalizeProfitLearning === 'function') {
              await this.learning.finalizeProfitLearning(e, {
                finalResult:String(q.result || '').toLowerCase(), terminalNetCents, reason:'market_final',
              });
            }
            if (!isHardStop) patch.researchTrackingComplete = true;
          } else {
            await this.market.ensureFreshBook?.(e.ticker, 5000).catch?.(() => null);
            const freshQ = this.market.getQuote(e.ticker) || q;
            const book = this.market.getBook?.(e.ticker);
            const bookMs = n(book?.updatedAtMs, n(freshQ?.updatedAtMs));
            const bookFresh = Boolean(book && bookMs > 0 && Date.now() - bookMs <= 5000 && !freshQ?.bookInvalid && n(freshQ?.yesBid) > 0);
            if (bookFresh && typeof this.market.executableBid === 'function') {
              ...
              if (exec?.full && n(exec.filled) + 1e-9 >= qty && Number.isFinite(Number(exec.avgCents))) {
                const executableBidCents = n(exec.avgCents);
                const executableNetCents = this.fullPositionCounterfactualNetCents(e, executableBidCents, s);
                if (typeof this.learning?.observeProfitPostExit === 'function') {
                  await this.learning.observeProfitPostExit(e, {
                    executableBidCents, executableNetCents, observedAtMs:bookMs,
                  });
                }
              }
            }
