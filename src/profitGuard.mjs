import { FEEDER_CONCEPTS, PORTFOLIO_CONCEPTS, ACTIVE_PORTFOLIO_CONCEPTS, ULTIMATE_STOP_GUARD, STOP_LOSS_WATCHDOG, stopLossWatchdogThresholdsForStakeCents, STOP_GUARD_RECOVERY_LEARNING, ULTIMATE_PROFIT_GUARD, APEX_PROFIT_GUARD, PROTECTED_RUNNER_INTELLIGENCE, PROFIT_LEARNING_INTELLIGENCE, ATHENA_EXIT_COMPATIBLE_REVISIONS, ATHENA_EXIT_INTELLIGENCE, GOLDEN_EYE, ATOMIC_THUNDER, INFINITY_BREAK, AURORA_EXECUTION, POST_EXIT_RESEARCH, LEGACY_STOP_LOSS_CENTS, calculateAuroraSnapshotFromFeeModel, kalshiGeneralTakerFeeEstimateCents } from './doctrine.mjs';
...
function isAthenaExitTradeEntry(entry) {
  if (!entry || FEEDER_CONCEPTS.has(entry.conceptName)) return false;
  // R64 migration is performed explicitly by ensureUniversalAthenaX1Authority
  // on the real protection path. Keep this predicate snapshot/state based so
  // retired creation-era guard evaluators remain deterministic and testable.
...
function athenaExitRevisionAdoptable(revision) {
  const rev=String(revision||'');
  if(!rev)return true;
  return ATHENA_EXIT_COMPATIBLE_REVISIONS.includes(rev);
}

function athenaExitPolicyRevisionSupported(entry) {
  return athenaExitRevisionAdoptable(athenaExitPolicyRevision(entry));
}

function athenaExitActivationMinimumNetPerOriginalContractCents(entry, settings = {}) {
  const frozenAthena=n(entry?.entryConfig?.athenaExit?.activationMinimumNetPerOriginalContractCents,NaN);
  if(Number.isFinite(frozenAthena)&&frozenAthena>0)return Math.max(0.01,frozenAthena);
  const frozenInfinity=n(entry?.entryConfig?.infinityBreak?.minimumNetPerOriginalContractCents,NaN);
  if(Number.isFinite(frozenInfinity)&&frozenInfinity>0)return Math.max(0.01,frozenInfinity);
  // Pre-R64 Justice Arrow had no Infinity snapshot and hard-coded its own X1
  // minimum. On universal migration it adopts the operator's current Infinity
  // minimum-cents setting, then freezes that value into athenaExit.
  if(ACTIVE_PORTFOLIO_CONCEPTS.has(String(entry?.conceptName||''))){
    const configured=n(settings?.infinityBreakMinNetPerOriginalContractCents,NaN);
    if(Number.isFinite(configured)&&configured>0)return Math.max(0.01,configured);
  }
  const economic=n(entry?.entryConfig?.economicTarget?.netPerOriginalContractCents,n(entry?.entryConfig?.athenaFire?.economicTarget?.netPerOriginalContractCents,NaN));
  if(Number.isFinite(economic)&&economic>0)return Math.max(0.01,economic);
  return Math.max(0.01,n(settings?.infinityBreakMinNetPerOriginalContractCents,INFINITY_BREAK.defaultMinimumNetPerOriginalContractCents));
}
...
export class ProfitGuard {
  ...
  async completeOpportunityEpisode(entry,{closedAtMs=null}={}) {
    ...
    const reason=String(entry?.closeReason||'');const pnl=n(entry?.pnlCents);const mae=n(entry?.maeCents);const targetPerContract=n(entry?.entryConfig?.athenaExit?.activationMinimumNetPerOriginalContractCents,n(entry?.entryConfig?.infinityBreak?.minimumNetPerOriginalContractCents,n(entry?.entryConfig?.athenaFire?.economicTarget?.netPerOriginalContractCents,n(entry?.entryConfig?.economicTarget?.netPerOriginalContractCents,5))));
    let label='EXPIRED_NO_IMPULSE';
    if(reason==='infinity_break')label=mae<=targetPerContract+1e-9?'CLEAN_BOLT':'TOXIC_LATE_BOLT';
    else if(reason==='athena_x1_exit'&&pnl>0)label=String(entry?.conceptName||'')==='Sagittarius Justice Arrow'?'CLEAN_BOLT':(mae<=targetPerContract+1e-9?'CLEAN_BOLT':'TOXIC_LATE_BOLT');
  ...
  athenaExitActivationPolicy(entry, settings = this.getSettings()) {
    const minimumNetPerOriginalContractCents=athenaExitActivationMinimumNetPerOriginalContractCents(entry,settings);
    const originalCount=Math.max(1e-9,n(entry?.count,remainingCount(entry)));
    return {
      minimumNetPerOriginalContractCents,
      targetAggregateNetCents:minimumNetPerOriginalContractCents*originalCount,
      originalCount,
      thresholdSetting:'infinityBreakMinNetPerOriginalContractCents',
      fullExecutableDepthRequired:true,
      latch:true,
    };
  }

  async ensureUniversalAthenaX1Authority(entry, settings = this.getSettings()) {
    if(!entry||entry.status!=='open'||!ACTIVE_PORTFOLIO_CONCEPTS.has(String(entry.conceptName||'')))return entry;
    const cfg=entry.entryConfig&&typeof entry.entryConfig==='object'?entry.entryConfig:{};
    const activation=this.athenaExitActivationPolicy(entry,settings);
    const existing=cfg.athenaExit&&typeof cfg.athenaExit==='object'?cfg.athenaExit:{};
    const currentAuthority=String(cfg.profitAuthority||'');
    const currentRevision=String(cfg.profitAuthorityRevision||'');
    const alreadyUniversal=currentAuthority===ATHENA_EXIT_INTELLIGENCE.version
      && currentRevision===ATHENA_EXIT_INTELLIGENCE.policyRevision
      && n(existing.activationMinimumNetPerOriginalContractCents)>0
      && !cfg.infinityBreak;
    if(alreadyUniversal)return entry;
    // Unknown X1 revisions stay frozen so ProfitGuard can fail closed in the
    // profit domain. Compatible R1 snapshots are rewritten to R2 below.
    if(currentAuthority===ATHENA_EXIT_INTELLIGENCE.version
      && currentRevision
      && !athenaExitRevisionAdoptable(currentRevision)){
      return entry;
    }
    const priorAuthority=String(cfg.profitAuthority||cfg.infinityBreak?.version||'');
    const migrated={...cfg,
      profitAuthority:ATHENA_EXIT_INTELLIGENCE.version,
      profitAuthorityRevision:ATHENA_EXIT_INTELLIGENCE.policyRevision,
      athenaExit:{
        ...existing,version:ATHENA_EXIT_INTELLIGENCE.version,policyRevision:ATHENA_EXIT_INTELLIGENCE.policyRevision,
        fullPositionOnly:true,positionSplitting:false,maximumExecutableBookAgeMs:Number(ATHENA_EXIT_INTELLIGENCE.maximumExecutableBookAgeMs),
        activationThresholdSetting:activation.thresholdSetting,
        activationMinimumNetPerOriginalContractCents:activation.minimumNetPerOriginalContractCents,
        activationRequiresFreshFullPositionExecutableDepth:true,activationLatch:true,lossAuthority:'U-SG1',
      },
      profitAuthorityMigration:{version:'R64-UNIVERSAL-ATHENA-X1',fromProfitAuthority:priorAuthority||null,migratedAtMs:Date.now()},
    };
    delete migrated.infinityBreak;
    // Preserve the entry/source/continuation railway while correcting only the
    // post-entry profit leg. This field is telemetry, not execution authority.
    migrated.authorityChain=String(cfg.authorityChain||'').replaceAll('INFINITY_BREAK/AURORA','ATHENA_X1/AURORA').replaceAll('INFINITY BREAK/AURORA','ATHENA_X1/AURORA');
    if(!migrated.authorityChain){
      migrated.authorityChain=entry.conceptName==='Sagittarius Justice Arrow'
        ?'ANOTHER_DIMENSION_WIN->SAGITTARIUS_JUSTICE_ARROW->HARD_EXECUTION_SAFETY->ATHENA_X1/AURORA'
        :entry.conceptName==='Scarlet Needle'
          ?'PROFITABLE_ATHENA_X1_CLOSE->SCARLET_NEEDLE->HARD_EXECUTION_SAFETY->ATHENA_X1/AURORA'
          :'COSMO_SHADOW->COSMO_GREEN->ATOMIC_THUNDER_BOLT->ATHENA->ATTACK->ATHENA_X1/AURORA';
    }
    try{
      await this.db.updateEntry(entry.id,{entryConfig:migrated,updatedAtMs:Date.now()});
      entry={...entry,entryConfig:migrated};
      await this.audit('athena_x1_universal_authority_migrated',{id:entry.id,ticker:entry.ticker,concept:entry.conceptName,fromProfitAuthority:priorAuthority||null,activationMinimumNetPerOriginalContractCents:activation.minimumNetPerOriginalContractCents,lossAuthority:'U-SG1'});
    }catch(error){
      // Fail closed on profit authority without disabling Aurora: keep the
      // corrected authority in-memory for this protection pass and retry the
      // durable migration on the next pass/restart.
      entry={...entry,entryConfig:migrated};
      await this.audit('athena_x1_universal_authority_migration_persistence_failed',{id:entry.id,ticker:entry.ticker,concept:entry.conceptName,error:String(error?.message||error)},'error').catch(()=>{});
    }
    return entry;
  }
  ...
  async persistAthenaExitState(entry, state) {
    ...
    const compatibleFrozen=athenaExitRevisionAdoptable(frozenRevision);
    const compatibleState=athenaExitRevisionAdoptable(stateRevision)
      || String(state?.version||'')===ATHENA_EXIT_INTELLIGENCE.version;
    if(!compatibleFrozen||!compatibleState){
  ...
  async persistAthenaExitStateFailClosed(entry, state, stage='tracking') {
    try{
      return{ok:true,state:await this.persistAthenaExitState(entry,state),error:null};
    }catch(error){
      await this.audit('athena_x1_state_persistence_failed',{
        id:entry.id,ticker:entry.ticker,concept:entry.conceptName,stage,
        error:String(error?.message||error),
      },'error').catch(()=>{});
      // Never activate, advance, or commit a profit-side exit from state that
      // was not durably written. The caller keeps the last persisted X1 state
      // (if any), while Aurora continues to run independently on every pass.
      return{ok:false,state:activeAthenaExitState(entry),error};
    }
  }
  ...
  async evaluateAthenaExit(entry, q, settings) {
    ...
    const activationLatched=committed||Boolean(state?.activationLatched&&n(state?.activationReachedAtMs)>0);
    const activationMeta={
      activationMinimumNetPerOriginalContractCents:activation.minimumNetPerOriginalContractCents,
      activationTargetAggregateNetCents:activation.targetAggregateNetCents,
      activationThresholdSetting:activation.thresholdSetting,
      activationLatched,
    };
    ...
    if(!bookFresh){
      ...
      if(state&&!committed&&activationLatched){
        const persisted=await this.persistAthenaExitStateFailClosed(entry,{...state,...activationMeta,executableFull:false,dataHoldReason:'stale_or_invalid_book',breakEvenPriceCents},'stale_book_hold');
        persistenceFailed=!persisted.ok;state=persisted.state||state;
      }
      return{q:freshQ,state:activationLatched?state:null,armed:activationLatched,committed,triggered:false,executable:false,executableFull:false,breakEvenPriceCents,persistenceFailed,...activationMeta,...athenaExitTelemetry(activationLatched?state:null)};
    }
    ...
    if(!executableFull){
      let persistenceFailed=false;
      if(state&&!committed&&activationLatched){
        const persisted=await this.persistAthenaExitStateFailClosed(entry,{...state,...activationMeta,executableFull:false,dataHoldReason:'partial_executable_depth',breakEvenPriceCents},'partial_depth_hold');
        persistenceFailed=!persisted.ok;state=persisted.state||state;
      }
      return{q:freshQ,state:activationLatched?state:null,armed:activationLatched,committed,triggered:false,executable:false,executableFull:false,breakEvenPriceCents,persistenceFailed,...activationMeta,...athenaExitTelemetry(activationLatched?state:null)};
    }
    ...
    const activationReached=executableNetCents+1e-9>=activation.targetAggregateNetCents;

    if(committed){
      const executable=executableNetCents>=-1e-9;
      return{q:freshQ,state,armed:true,committed:true,triggered:executable,executable,executableFull:true,executableBidCents,executableNetCents,breakEvenPriceCents,activationReached,...activationMeta,activationLatched:true,...athenaExitTelemetry(state)};
    }
    // The retained Infinity Break minimum-cents control is now the universal
    // ATHENA-X1 activation gate. Before the first fresh full-position
    // executable observation reaches that frozen economic target, X1 has no
    // profit-side state and therefore cannot harvest early. Aurora remains
    // fully active because its loss-domain evaluation occurs independently.
    if(!activationLatched&&!activationReached){
      return{q:freshQ,state:null,armed:false,committed:false,triggered:false,executable:true,executableFull:true,executableBidCents,executableNetCents,breakEvenPriceCents,activationReached:false,...activationMeta,activationLatched:false};
    }
    if(activationLatched&&observedBookMs<=n(state?.lastObservedBookMs)){
      return{q:freshQ,state,armed:true,committed:false,triggered:false,executable:true,executableFull:true,executableBidCents,executableNetCents,breakEvenPriceCents,activationReached,...activationMeta,activationLatched:true,...athenaExitTelemetry(state)};
    }
    ...
    let advanced;
    try{
      // A pre-R64, non-committed X1 state did not have the operator activation
      // covenant. Start the universal X1 path from the threshold-crossing book
      // rather than inheriting observations collected below the new gate.
      advanced=advanceAthenaExitState(activationLatched?state:null,{
    ...
    const activatedAtMs=activationLatched?n(state?.activationReachedAtMs,observedBookMs):observedBookMs;
    const persisted=await this.persistAthenaExitStateFailClosed(entry,{
      ...advanced.state,...activationMeta,
      activationReachedAtMs:activatedAtMs,activationLatched:true,
      executableFull:true,dataHoldReason:null,breakEvenPriceCents,
    },activationLatched?'advance':'activate');
    if(!persisted.ok){
      state=persisted.state||state;
      return{
        q:freshQ,state:activationLatched?state:null,armed:activationLatched,committed:false,triggered:false,
        executable:true,executableFull:true,executableBidCents,executableNetCents,breakEvenPriceCents,activationReached,
        persistenceFailed:true,...activationMeta,activationLatched,...athenaExitTelemetry(activationLatched?state:null),
      };
    }
    state=persisted.state;
    if(!activationLatched){
      await this.audit('athena_x1_activated',{
        id:entry.id,ticker:entry.ticker,concept:entry.conceptName,
        thresholdSetting:activation.thresholdSetting,
        activationMinimumNetPerOriginalContractCents:activation.minimumNetPerOriginalContractCents,
        activationTargetAggregateNetCents:activation.targetAggregateNetCents,
        executableNetCents,executableBidCents,observedBookMs,
      });
    }
    if(advanced.newPeak){
    ...
    const executable=executableNetCents>=-1e-9;
    return{q:freshQ,state,armed:true,committed:advanced.decision==='EXIT',triggered:advanced.decision==='EXIT'&&executable,executable,executableFull:true,executableBidCents,executableNetCents,breakEvenPriceCents,activationReached,...activationMeta,activationLatched:true,...athenaExitTelemetry(state)};
  }
  ...
  async applyExitFill(entry, decision, { fillCount, fillPriceCents, exitFeeCents = 0, orderId = null, clearClientId = true }) {
    ...
    // Scarlet continuation follows a profitable close by the current universal
    // profit authority. Keep the legacy Infinity Break predicate only so an
    // exit already committed before an R64 deployment can finish without
    // losing its one-shot continuation authorization.
    if(remaining<=1e-9&&pnl>0&&(this.isAthenaExitDecision(closedEntry,decision)||this.isInfinityBreakExitDecision(closedEntry,decision)))this.notifyPositionClosed(closedEntry);
  ...
  async _protect(entry) {
    ...
    // R64 universal profit-authority migration runs only after all pre-existing
    // exit_pending broker obligations above have been reconciled. This means an
    // upgrade never cancels an order that may already have filled, while every
    // still-open current Execution Attack is switched to ATHENA-X1 before any
    // ordinary profit authority can evaluate it.
    entry=await this.ensureUniversalAthenaX1Authority(entry,s);
    ...
    // Legacy creation-time Infinity Break / Atomic Thunder compatibility only.
    // R64 current real Execution Attacks have already been migrated to ATHENA-X1
    // above and cannot enter this branch; pre-existing non-current concepts retain
    // their frozen authority without silently rewriting historical behavior.
    if (!this.isAthenaExitTrade(entry)
      && (this.isInfinityBreakTrade(entry) || this.isAtomicThunderTrade(entry))
      && this.atomicThunderPolicy(entry, s).enabled) {
    ...
    // Legacy creation-time Golden Eye compatibility only. Current R64 real
    // Execution Attacks are excluded above and in the engine-level aggregate
    // cashout selector, so Golden Eye cannot race ATHENA-X1 for their profit side.
    if (!this.isAthenaExitTrade(entry) && this.isGoldenEyeTrade(entry)) {
      const gd = {
    ...
    // R64 ATHENA-X1 is the universal profit authority for every current real
    // Execution Attack. U-SG1/Aurora has already run above, so X1 can reason
    // about 100% of a profitable position without acquiring loss-domain authority
    // or bypassing settlement, executable-depth, or reconciliation safeguards.
    if (this.isAthenaExitTrade(entry)) {
  ...
  async manualCashout(entry, q, { reason = 'manual_cashout' } = {}) {
    return this.withEntryLockQueued(entry.id, async () => {
      ...
      if(goldenEyeExit&&ACTIVE_PORTFOLIO_CONCEPTS.has(String(entry?.conceptName||''))){
        await this.audit('golden_eye_cashout_skipped_athena_x1_priority',{id:entry.id,ticker:entry.ticker,concept:entry.conceptName,profitAuthority:ATHENA_EXIT_INTELLIGENCE.version});
        return {closed:false,skipped:'athena_x1_has_priority'};
      }
