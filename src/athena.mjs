    ...
    if(!bolt||!bolt.id){reason='bolt_required';this.reject+=1;}
      ...
      if(immediate){
        if(settings.athenaSoulEnabled===true){
          ...
          if(!admitted.ok){decision='REJECT';reason=admitted.reason;this.reject+=1;}
          else{decision='FIRE';reason='cosmo_green_best_attack';this.fire+=1;context=Object.assign({},context,{soulStory:admitted.story});this.persistSoul().catch(()=>{});}
        }else{decision='FIRE';reason='cosmo_green_best_attack';this.fire+=1;}
      }
      else this.reject+=1;
    }
