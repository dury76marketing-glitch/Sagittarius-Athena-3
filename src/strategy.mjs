...
import { athenaExclamationPrimeReview } from './athenaExclamation.mjs';
...
export function validateAthenaFireCommand(command,{concept,q,settings,now=Date.now()}={}){
  ...
  // R64: the operator's Infinity Break minimum-cents setting is retained as the
  // universal ATHENA-X1 activation threshold for every real Execution Attack,
  // including Sagittarius Justice Arrow. It is no longer an ordinary sell rule.
  const configuredTarget=Number(settings?.infinityBreakMinNetPerOriginalContractCents??INFINITY_BREAK.defaultMinimumNetPerOriginalContractCents);
...
export function entryConfigSnapshot(settings, conceptName, sourceFeeder = null, actualStakeCents = null, sourceEntryConfig = null, gameClockState = null, eventTicker = '') {
  ...
  const attackMap={
    ...
    'Athena Exclamation':{stake:Number(settings.athenaExclamationStakeCents),min:Number(settings.athenaExclamationMinEntryCents),max:Number(settings.athenaExclamationMaxEntryCents),structuralRole:'POST_12_JUSTICE_ARROWS_GEMINI_DIVERSION'},
    'Lightning Plasma':{stake:Number(settings.lightningPlasmaFieldStakeCents),min:Number(settings.lightningPlasmaMinEntryCents),max:Number(settings.lightningPlasmaMaxEntryCents),maxStrikes:Number(LIGHTNING_PLASMA.maxStrikes),fieldBudgetSharedAcrossStrikes:true,oneStrikePerTicker:true,minimumUpliftCents:LIGHTNING_PLASMA.minimumUpliftCents},
  };
  ...
  const athenaExclamationContinuation=conceptName==='Athena Exclamation';
  const lightningPlasmaGemini=conceptName==='Lightning Plasma';
  const realExecutionAttack=ACTIVE_PORTFOLIO_CONCEPTS.has(conceptName);
  const authorityChain=justiceContinuation?'ANOTHER_DIMENSION_WIN->SAGITTARIUS_JUSTICE_ARROW->HARD_EXECUTION_SAFETY->ATHENA_X1/AURORA'
    :athenaExclamationContinuation?'12_JUSTICE_ARROWS->NEXT_ANOTHER_DIMENSION_WIN->ATHENA_EXCLAMATION->HARD_EXECUTION_SAFETY->ATHENA_X1/AURORA'
    :lightningPlasmaGemini?'GEMINI->ANOTHER_DIMENSION_REFERENCE_PLUS_4C->LIGHTNING_PLASMA_SLOT->HARD_EXECUTION_SAFETY->ATHENA_X1/AURORA'
    :scarletContinuation?'PROFITABLE_ATHENA_X1_CLOSE->SCARLET_NEEDLE->HARD_EXECUTION_SAFETY->ATHENA_X1/AURORA'
    :'COSMO_SHADOW->COSMO_GREEN->ATOMIC_THUNDER_BOLT->ATHENA->ATTACK->ATHENA_X1/AURORA';
  const strategicEntryAuthority=justiceContinuation?SAGITTARIUS_JUSTICE_ARROW.strategicEntryAuthority
    :athenaExclamationContinuation?ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority
    :lightningPlasmaGemini?LIGHTNING_PLASMA.strategicEntryAuthority
    :scarletContinuation?SCARLET_NEEDLE.strategicEntryAuthority:ATHENA_COMMANDER.version;
  return {
    release:RELEASE,
    authorityChain,
    strategicEntryAuthority,
    profitAuthority:realExecutionAttack?ATHENA_EXIT_INTELLIGENCE.version:null,
    profitAuthorityRevision:realExecutionAttack?ATHENA_EXIT_INTELLIGENCE.policyRevision:null,
    lossAuthority:realExecutionAttack?AURORA_EXECUTION.lossAuthority:null,
...
export function lightningPlasmaSourceObservedAtMs(entry) {
  if (!entry || String(entry.conceptName || '') !== 'Another Dimension') return 0;
...
export function lightningPlasmaStrikeQualification(source, quote, settings, now = Date.now()) {
  ...
  const maxSpreadCents = Number(settings?.maxSpreadCents ?? LIGHTNING_PLASMA.maxSpreadCents);
  const minimumUpliftCents = Number(LIGHTNING_PLASMA.minimumUpliftCents);
  const ageMs = observedAtMs > 0 ? Math.max(0, Number(now) - observedAtMs) : null;
  const base = { observedAtMs, anchorCents, bidCents:bid, askCents:ask, ageMs, minEntryCents, maxEntryCents, maxSpreadCents, minimumUpliftCents, noTimeWindow:true };
  if (!source || String(source.conceptName || '') !== 'Another Dimension') return { ...base, ok:false, reason:'another_dimension_reference_required' };
  if (String(source.status || '') === 'rejected') return { ...base, ok:false, reason:'rejected_another_dimension_reference' };
  if (!(anchorCents > 0)) return { ...base, ok:false, reason:'another_dimension_entry_reference_missing' };
  ...
  const upliftCents = bid - anchorCents;
  if (upliftCents + 1e-9 < minimumUpliftCents) return { ...base, ok:false, reason:'uplift_below_four_cents', upliftCents };
  // Stronger uplift ranks ahead. There is intentionally no age/window term:
  // LP2 may fill a free strike slot whenever an active market later reaches +4c.
  const score = 100 + Math.min(40, Math.max(0, upliftCents) * 5) + Math.max(0, (maxSpreadCents - spreadCents) * 4);
  return { ...base, ok:true, reason:'qualified', spreadCents, upliftCents, score };
}

export function lightningPlasmaFieldId(sources = []) {
  const identity = (sources || []).map((x) => String(x?.id || '')).filter(Boolean).sort().join('|');
  if (!identity) return null;
  return `LP2-${createHash('sha256').update(identity).digest('hex').slice(0, 16)}`;
}

export function lightningPlasmaFieldSelection(entries, marketMap, settings, now = Date.now()) {
  const maxStrikes = Number(LIGHTNING_PLASMA.maxStrikes);
  // Gemini may contain multiple historical Another Dimension references for the
  // same exact market. A real Lightning Plasma field may use only one candidate
  // per ticker at a time, selecting the strongest/current reference deterministically.
  const byTicker = new Map();
  for (const source of entries || []) {
    if (String(source?.conceptName || '') !== 'Another Dimension') continue;
    const ticker = String(source?.ticker || '');
    if (!ticker) continue;
    const quote = marketMap?.get?.(ticker);
    if (!quote) continue;
    const qualification = lightningPlasmaStrikeQualification(source, quote, settings, now);
    if (!qualification.ok) continue;
    const row = { source, quote, qualification, ticker, eventTicker:String(source.eventTicker || quote.eventTicker || ticker) };
    const prior = byTicker.get(ticker);
    if (!prior
        || Number(row.qualification.upliftCents) > Number(prior.qualification.upliftCents)
        || (Number(row.qualification.upliftCents) === Number(prior.qualification.upliftCents) && lightningPlasmaSourceObservedAtMs(source) > lightningPlasmaSourceObservedAtMs(prior.source))
        || (Number(row.qualification.upliftCents) === Number(prior.qualification.upliftCents) && lightningPlasmaSourceObservedAtMs(source) === lightningPlasmaSourceObservedAtMs(prior.source) && String(source.id || '').localeCompare(String(prior.source.id || '')) < 0)) {
      byTicker.set(ticker, row);
    }
  }
  const candidates = [...byTicker.values()].sort((a,b) =>
  ...
  return {
    ...
    policyRevision: LIGHTNING_PLASMA.policyRevision,
    fieldId: lightningPlasmaFieldId(candidates.map((x)=>x.source)),
    sourceCount: (entries || []).filter((e)=>String(e?.conceptName||'')==='Another Dimension').length,
    independentMarketCount: candidates.length,
    maxStrikes,
    minimumUpliftCents: Number(LIGHTNING_PLASMA.minimumUpliftCents),
    noTimeWindow: true,
    qualifies: candidates.length > 0,
    candidates,
  };
}
...
function athenaR2ModelSettings(settings={},concept=''){
  ...
  else if(concept==='Lightning Plasma')Object.assign(out,{modelMinEntry:Number(settings.lightningPlasmaMinEntryCents),modelMaxEntry:Number(settings.lightningPlasmaMaxEntryCents),modelMaxSpread:Number(settings.maxSpreadCents),modelMinStrikes:1,modelMaxStrikes:Number(LIGHTNING_PLASMA.maxStrikes),modelRise:Number(LIGHTNING_PLASMA.minimumUpliftCents)});
  return out;
}
...
export class StrategyEngine {
  ...
  resourceSnapshot() {
    return {
      ...
      athenaExclamation:{version:ATHENA_EXCLAMATION_DOCTRINE.version,policyRevision:ATHENA_EXCLAMATION_DOCTRINE.policyRevision,role:ATHENA_EXCLAMATION_DOCTRINE.role,triggerEveryJusticeArrows:ATHENA_EXCLAMATION_DOCTRINE.triggerEveryJusticeArrows},
    };
  }
  ...
  // R65: AE1 Gold-Saint convergence is retired. Athena Exclamation is now
  // invoked only by the durable Gemini continuation router after twelve
  // successfully opened Sagittarius Justice Arrows.

  async observeGoldSaintQualification() {
    return {event:null,entry:null,recorded:false,reason:'ae1_three_saint_convergence_retired'};
  }
  ...
  async revalidateHunterEntryDoctrine(concept, executionQuote, executionPlan, settings, {
    ...
    if (concept === 'Athena Exclamation') return { ...boundary, ok:false, reason:'ae1_legacy_creation_path_retired' };
    ...
    if (concept === 'Lightning Plasma') return { ...boundary, ok:false, reason:'lp1_legacy_creation_path_retired' };
  ...
  async createHunter(concept, q, stakeCents, _legacyStopLossCents = 0, { sourceFeeder = null, sourceTradeId = null, sourceEntryConfig = null, recoverySourceSnapshot = null, crashSourceSnapshot = null, entryQualificationSnapshot = null, athenaFireCommand = null, legacyCompatibility = false } = {}) {
    ...
    try {
      ...
      if(newGenerationEntry){
        ...
        const exclamationAuthorityMode=String(athenaFireCommand?.authorityMode||'')===ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority;
        const exclamationLineage=athenaFireCommand?.decisionEvidence?.athenaExclamationContinuation;
        const plasmaAuthorityMode=String(athenaFireCommand?.authorityMode||'')===LIGHTNING_PLASMA.strategicEntryAuthority;
        const plasmaLineage=athenaFireCommand?.decisionEvidence?.lightningPlasmaGemini;
        ...
        if(concept==='Athena Exclamation'&&(!exclamationAuthorityMode||!exclamationLineage?.authorizationId||!exclamationLineage?.parentShadowTradeId||Number(exclamationLineage?.justiceArrowCount||0)<ATHENA_EXCLAMATION_DOCTRINE.triggerEveryJusticeArrows)){
          trace('FIRE_EXECUTION_ENVELOPE','BLOCKED','athena_exclamation_cycle_authority_required');
          await this.audit('athena_exclamation_execution_blocked',{concept,ticker:q.ticker,eventTicker:expectedEventTicker,reason:'athena_exclamation_cycle_authority_required'});
          return null;
        }
        if(concept!=='Athena Exclamation'&&exclamationAuthorityMode){
          trace('FIRE_EXECUTION_ENVELOPE','BLOCKED','athena_exclamation_authority_attack_mismatch');
          return null;
        }
        if(concept==='Lightning Plasma'&&(!plasmaAuthorityMode||!plasmaLineage?.authorizationId||!plasmaLineage?.parentShadowTradeId||Number(plasmaLineage?.upliftCents||0)+1e-9<Number(LIGHTNING_PLASMA.minimumUpliftCents))){
          trace('FIRE_EXECUTION_ENVELOPE','BLOCKED','lightning_plasma_gemini_authority_required');
          await this.audit('lightning_plasma_execution_blocked',{concept,ticker:q.ticker,eventTicker:expectedEventTicker,reason:'lightning_plasma_gemini_authority_required'});
          return null;
        }
      ...
      if(newGenerationEntry){
        ...
        const exclamationContinuationAuthority=concept==='Athena Exclamation'&&String(athenaFireCommand?.authorityMode||'')===ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority;
        const plasmaGeminiAuthority=concept==='Lightning Plasma'&&String(athenaFireCommand?.authorityMode||'')===LIGHTNING_PLASMA.strategicEntryAuthority;
        const dedicatedContinuationAuthority=scarletContinuationAuthority||justiceContinuationAuthority||exclamationContinuationAuthority||plasmaGeminiAuthority;
        const authorityMode=scarletContinuationAuthority?SCARLET_NEEDLE.strategicEntryAuthority:justiceContinuationAuthority?SAGITTARIUS_JUSTICE_ARROW.strategicEntryAuthority:exclamationContinuationAuthority?ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority:plasmaGeminiAuthority?LIGHTNING_PLASMA.strategicEntryAuthority:'ATHENA_A3';
        ...
        const reason=scarletContinuationAuthority?'scarlet_post_profit_continuation_authority':justiceContinuationAuthority?'justice_arrow_post_shadow_victory_authority':exclamationContinuationAuthority?'athena_exclamation_post_12_arrows_authority':plasmaGeminiAuthority?'lightning_plasma_gemini_uplift_authority':'supreme_fire_authority';
        trace('ATHENA','PASS',reason,{boltId:athenaFireCommand.boltId,selectedAttack:concept});
      ...
      if (concept==='Athena Exclamation') frozenEntryConfig.athenaExclamation={version:ATHENA_EXCLAMATION_DOCTRINE.version,policyRevision:ATHENA_EXCLAMATION_DOCTRINE.policyRevision,continuation:structuredClone(entryQualificationSnapshot?.athenaExclamationContinuation||null)};
      if (concept==='Lightning Plasma') frozenEntryConfig.lightningPlasmaGemini=structuredClone(entryQualificationSnapshot?.lightningPlasmaGemini||null);
      ...
      if(ACTIVE_PORTFOLIO_CONCEPTS.has(concept)){
        // R64 universal profit doctrine: every real Execution Attack freezes
        // ATHENA-X1 as its only normal profit authority. The operator's existing
        // Infinity Break minimum-cents setting survives only as X1's immutable
        // activation threshold for this position; it is not a competing sell
        // authority. Another Dimension is created by its separate Gemini path
        // and never reaches this real-Execution-Attack snapshot.
        ...
        frozenEntryConfig.athenaExit={
          ...
          activationMinimumNetPerOriginalContractCents:frozenEconomicTargetNet,
  ...
  async executeJusticeArrowContinuation(q, parentShadow, { authorizationId, authorizedAtMs=Date.now() } = {}) {
    ...
    const lineage={
      ...
      athenaExclamationCycleVersion:ATHENA_EXCLAMATION_DOCTRINE.version,
      athenaExclamationCyclePolicyRevision:ATHENA_EXCLAMATION_DOCTRINE.policyRevision,
  ...
  async executeAthenaExclamationContinuation(q, parentShadow, { authorizationId, authorizedAtMs=Date.now(), cycleState=null } = {}) {
    const s=this.getSettings();
    if(s.athenaExclamationEnabled!==true)return null;
    if(parentShadow?.conceptName!=='Another Dimension'||parentShadow?.status!=='closed'||Number(parentShadow?.remainingCount||0)>1e-9)return null;
    if(String(parentShadow?.closeReason||'')!==ANOTHER_DIMENSION.profitableCloseReason||!(Number(parentShadow?.pnlCents||0)>0))return null;
    if(!cycleState?.armed||Number(cycleState?.justiceArrowCount||0)<Number(ATHENA_EXCLAMATION_DOCTRINE.triggerEveryJusticeArrows))return null;
    const ticker=String(q?.ticker||''),parentTicker=String(parentShadow?.ticker||'');
    if(!ticker||ticker!==parentTicker)return null;
    const parentSide=String(parentShadow?.side||parentShadow?.entryConfig?.side||'YES').toUpperCase();
    if(parentSide!=='YES')return null;
    const ask=Math.max(0,Number(q?.yesAsk||0)),bid=Math.max(0,Number(q?.yesBid||0));
    if(!(ask>0)||!(bid>0)||bid>ask)return null;
    const minEntryCents=Number(s.athenaExclamationMinEntryCents),maxEntryCents=Number(s.athenaExclamationMaxEntryCents),stakeCents=Number(s.athenaExclamationStakeCents);
    const target=justiceArrowEconomicTarget({askCents:ask,stakeCents,settings:s});
    if(!target.targetFeasible)return null;
    const at=Math.max(1,Number(authorizedAtMs)||Date.now());
    const id=String(authorizationId||`ATHENA-EXCLAMATION:${parentShadow.id}:1`);
    const review=athenaExclamationPrimeReview({parentShadow,quote:q,executionPlan:{averagePriceCents:ask,bestAskCents:ask},settings:s,cycleState,now:at});
    if(!review?.ok)return null;
    const lineage={
      version:ATHENA_EXCLAMATION_DOCTRINE.version,
      policyRevision:ATHENA_EXCLAMATION_DOCTRINE.policyRevision,
      authority:ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority,
      authorizationId:id,
      parentShadowTradeId:String(parentShadow.id),
      parentConcept:String(parentShadow.conceptName||''),
      sourceCosmo:String(parentShadow.sourceFeeder||parentShadow.entryConfig?.sourceFeeder||''),
      ...
      parentRealizedPnlCents:Number(parentShadow.pnlCents||0),
      parentCloseReason:String(parentShadow.closeReason||''),
      justiceArrowCount:Number(cycleState.justiceArrowCount||0),
      cycleStartedAtMs:Number(cycleState.cycleStartedAtMs||0),
      lastAthenaExclamationEntryId:cycleState.lastAthenaExclamationEntryId||null,
      authorizedAtMs:at,sameTicker:true,sameSide:true,ordinaryCooldownBypassed:true,fullExecutionSafetyRequired:true,
      profitAuthority:ATHENA_EXIT_INTELLIGENCE.version,
      lossAuthority:AURORA_EXECUTION.lossAuthority,
    };
    const ranking=[{concept:'Athena Exclamation',displayName:EXECUTION_ATTACK_DISPLAY['Athena Exclamation']?.name||'Athena Exclamation',score:100,authorityMode:ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority,targetFeasible:target.targetFeasible,requiredTargetBidCents:target.requiredTargetBidCents}];
    const core={
      version:ATHENA_COMMANDER.version,policyRevision:ATHENA_COMMANDER.policyRevision,
      authorityMode:ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority,
      authoritySource:'POST_12_JUSTICE_ARROWS_NEXT_PROFITABLE_ANOTHER_DIMENSION_CLOSE',strategicSelectionBypassed:true,
      boltId:id,boltFingerprint:null,systemName:s.systemName,sourceRelease:RELEASE,
      decidedAtMs:at,expiresAtMs:at+5_000,ticker,eventTicker:String(q?.eventTicker||parentShadow?.eventTicker||ticker),side:'YES',
      selectedAttack:'Athena Exclamation',selectedAttackDisplay:EXECUTION_ATTACK_DISPLAY['Athena Exclamation']?.name||'Athena Exclamation',
      stakeCents,fieldBudgetCents:null,maxRays:null,operatorMinEntryCents:minEntryCents,operatorMaxEntryCents:maxEntryCents,
      entryPriceCents:ask,authorizedMaxEntryCents:ask,maxSpreadCents:Number(s.maxSpreadCents??3),auroraDamageControlPercent:Number(s.auroraDamageControlPercent??45),
      profitAuthority:ATHENA_EXIT_INTELLIGENCE.version,infinityBreakPolicyVersion:null,economicTarget:target,survivalCertificate:null,ranking,
      decisionEvidence:{
        athenaExclamationContinuation:lineage,
        anotherDimensionClose:{entryId:String(parentShadow.id),universe:'Gemini',concept:'Another Dimension',sourceCosmo:lineage.sourceCosmo,sourceCosmoTradeId:lineage.sourceCosmoTradeId,closeReason:String(parentShadow.closeReason||''),realizedPnlCents:Number(parentShadow.pnlCents||0),exitPriceCents:Number(parentShadow.exitPriceCents||0),closedAtMs:Number(parentShadow.closedAtMs||0)},
        configuredTargetNetPerOriginalContractCents:target.netPerOriginalContractCents,
        predictiveReVetoAllowed:false,normalStrategicDiscoveryBypassed:true,hardExecutionSafetyStillRequired:true,
      },
    ...
    const fireCommand=sealAthenaFireCommand(core);
    const decision={version:ATHENA_COMMANDER.version,policyRevision:ATHENA_COMMANDER.policyRevision,decision:'FIRE',reason:'athena_exclamation_post_12_justice_arrows',decidedAtMs:at,boltId:id,ticker,ranking,selectedAttack:'Athena Exclamation',selectedAttackDisplay:core.selectedAttackDisplay,fireCommand,economicObjective:'POST_12_JUSTICE_ARROWS_GEMINI_DIVERSION',configuredTargetNetPerOriginalContractCents:target.netPerOriginalContractCents,strategicAuthority:false,athenaExclamationContinuationAuthority:true,durableFireRequired:true,durableFirePersisted:true};
    const bolt={id,ticker,eventTicker:core.eventTicker,side:'YES',sport:String(parentShadow?.sport||'Unknown'),detectedAtMs:at,expiresAtMs:core.expiresAtMs,fingerprint:null,features:{bidCents:bid,askCents:ask,sport:String(parentShadow?.sport||'Unknown'),eligibleAttacks:ranking},greenTrigger:null,preBoltClearance:null,authorityMode:ATHENA_EXCLAMATION_DOCTRINE.strategicEntryAuthority};
    return this.executeAthenaFire(q,bolt,decision,{cosmos:[],athenaExclamationContinuation:lineage,anotherDimensionEntry:parentShadow});
  }

  async executeLightningPlasmaGeminiStrike(q, parentShadow, { authorizationId, authorizedAtMs=Date.now(), slotIndex=1, maxSlots=null, rayStakeCents=null, fieldBudgetCents=null } = {}) {
    const s=this.getSettings();
    if(s.lightningPlasmaEnabled!==true)return null;
    if(parentShadow?.conceptName!=='Another Dimension'||String(parentShadow?.status||'')==='rejected')return null;
    const ticker=String(q?.ticker||''),parentTicker=String(parentShadow?.ticker||'');
    if(!ticker||ticker!==parentTicker)return null;
    const parentSide=String(parentShadow?.side||parentShadow?.entryConfig?.side||'YES').toUpperCase();
    if(parentSide!=='YES')return null;
    const ask=Math.max(0,Number(q?.yesAsk||0)),bid=Math.max(0,Number(q?.yesBid||0));
    if(!(ask>0)||!(bid>0)||bid>ask)return null;
    const qualification=lightningPlasmaStrikeQualification(parentShadow,q,s,Date.now());
    if(!qualification.ok)return null;
    const configuredMaxSlots=Number(LIGHTNING_PLASMA.maxStrikes);
    const configuredBudget=Math.max(1,Math.floor(Number(s.lightningPlasmaFieldStakeCents||LIGHTNING_PLASMA.fieldStakeCents)));
    const slots=Math.max(1,Math.min(3,Math.floor(Number(maxSlots||configuredMaxSlots))));
    if(slots!==configuredMaxSlots)return null;
    const budget=Math.max(1,Math.floor(Number(fieldBudgetCents||configuredBudget)));
    if(budget!==configuredBudget)return null;
    // The operator's field budget is always divided by the configured number
    // of strike slots. Opening fewer than three markets must not grant the
    // first strike the unused capital from the other slots.
    const fixedRayStake=Math.max(1,Math.floor(configuredBudget/configuredMaxSlots));
    const stakeCents=Math.max(1,Math.floor(Number(rayStakeCents||fixedRayStake)));
    if(stakeCents!==fixedRayStake)return null;
    const minEntryCents=Number(s.lightningPlasmaMinEntryCents),maxEntryCents=Number(s.lightningPlasmaMaxEntryCents);
    const target=justiceArrowEconomicTarget({askCents:ask,stakeCents,settings:s});
    if(!target.targetFeasible)return null;
    const at=Math.max(1,Number(authorizedAtMs)||Date.now());
    const id=String(authorizationId||`LIGHTNING-PLASMA:${parentShadow.id}:4C`);
    const lineage={
      version:LIGHTNING_PLASMA.version,policyRevision:LIGHTNING_PLASMA.policyRevision,authority:LIGHTNING_PLASMA.strategicEntryAuthority,
      authorizationId:id,parentShadowTradeId:String(parentShadow.id),parentConcept:String(parentShadow.conceptName||''),
      sourceCosmo:String(parentShadow.sourceFeeder||parentShadow.entryConfig?.sourceFeeder||''),sourceCosmoTradeId:String(parentShadow.sourceTradeId||parentShadow.entryConfig?.sourceTradeId||''),
      referenceEntryCents:Number(qualification.anchorCents||0),observedBidCents:bid,observedAskCents:ask,upliftCents:Number(qualification.upliftCents||0),minimumUpliftCents:Number(LIGHTNING_PLASMA.minimumUpliftCents),
      noTimeWindow:true,slotIndex:Math.max(1,Math.min(configuredMaxSlots,Math.floor(Number(slotIndex)||1))),maxSlots:configuredMaxSlots,
      fieldBudgetCents:configuredBudget,rayStakeCents:fixedRayStake,fieldBudgetSharedAcrossStrikes:true,oneStrikePerTicker:true,
      authorizedAtMs:at,sameTicker:true,sameSide:true,ordinaryCooldownBypassed:true,fullExecutionSafetyRequired:true,
      profitAuthority:ATHENA_EXIT_INTELLIGENCE.version,lossAuthority:AURORA_EXECUTION.lossAuthority,
    };
    const ranking=[{concept:'Lightning Plasma',displayName:EXECUTION_ATTACK_DISPLAY['Lightning Plasma']?.name||'Lightning Plasma',score:100+Number(qualification.upliftCents||0),authorityMode:LIGHTNING_PLASMA.strategicEntryAuthority,targetFeasible:target.targetFeasible,requiredTargetBidCents:target.requiredTargetBidCents}];
    const core={
      version:ATHENA_COMMANDER.version,policyRevision:ATHENA_COMMANDER.policyRevision,authorityMode:LIGHTNING_PLASMA.strategicEntryAuthority,
      authoritySource:'GEMINI_ANOTHER_DIMENSION_PLUS_4C_UPLIFT',strategicSelectionBypassed:true,boltId:id,boltFingerprint:null,systemName:s.systemName,sourceRelease:RELEASE,
      decidedAtMs:at,expiresAtMs:at+5_000,ticker,eventTicker:String(q?.eventTicker||parentShadow?.eventTicker||ticker),side:'YES',selectedAttack:'Lightning Plasma',selectedAttackDisplay:EXECUTION_ATTACK_DISPLAY['Lightning Plasma']?.name||'Lightning Plasma',
      stakeCents,fieldBudgetCents:configuredBudget,maxRays:configuredMaxSlots,operatorMinEntryCents:minEntryCents,operatorMaxEntryCents:maxEntryCents,entryPriceCents:ask,authorizedMaxEntryCents:ask,maxSpreadCents:Number(s.maxSpreadCents??3),auroraDamageControlPercent:Number(s.auroraDamageControlPercent??45),
      profitAuthority:ATHENA_EXIT_INTELLIGENCE.version,infinityBreakPolicyVersion:null,economicTarget:target,survivalCertificate:null,ranking,
      decisionEvidence:{lightningPlasmaGemini:lineage,anotherDimensionReference:{entryId:String(parentShadow.id),universe:'Gemini',concept:'Another Dimension',referenceEntryCents:Number(parentShadow.entryPriceCents||0),status:String(parentShadow.status||'')},configuredTargetNetPerOriginalContractCents:target.netPerOriginalContractCents,predictiveReVetoAllowed:false,normalStrategicDiscoveryBypassed:true,hardExecutionSafetyStillRequired:true},
    };
    const fireCommand=sealAthenaFireCommand(core);
    const decision={version:ATHENA_COMMANDER.version,policyRevision:ATHENA_COMMANDER.policyRevision,decision:'FIRE',reason:'lightning_plasma_gemini_plus_4c',decidedAtMs:at,boltId:id,ticker,ranking,selectedAttack:'Lightning Plasma',selectedAttackDisplay:core.selectedAttackDisplay,fireCommand,economicObjective:'GEMINI_UPLIFT_THREE_SLOT_STRIKE',configuredTargetNetPerOriginalContractCents:target.netPerOriginalContractCents,strategicAuthority:false,lightningPlasmaGeminiAuthority:true,durableFireRequired:true,durableFirePersisted:true};
    const bolt={id,ticker,eventTicker:core.eventTicker,side:'YES',sport:String(parentShadow?.sport||'Unknown'),detectedAtMs:at,expiresAtMs:core.expiresAtMs,fingerprint:null,features:{bidCents:bid,askCents:ask,sport:String(parentShadow?.sport||'Unknown'),eligibleAttacks:ranking},greenTrigger:null,preBoltClearance:null,authorityMode:LIGHTNING_PLASMA.strategicEntryAuthority};
    return this.executeAthenaFire(q,bolt,decision,{cosmos:[],lightningPlasmaGemini:lineage,anotherDimensionEntry:parentShadow});
  }
  ...
  async executeAthenaFire(q, bolt, decision, context = {}) {
    ...
    const athenaExclamationContinuation=concept==='Athena Exclamation'?structuredClone(context?.athenaExclamationContinuation||command?.decisionEvidence?.athenaExclamationContinuation||null):null;
    const lightningPlasmaGemini=concept==='Lightning Plasma'?structuredClone(context?.lightningPlasmaGemini||command?.decisionEvidence?.lightningPlasmaGemini||null):null;
    const geminiContinuation=justiceArrowContinuation||athenaExclamationContinuation||lightningPlasmaGemini;
    const anotherDimensionEntry=geminiContinuation?(context?.anotherDimensionEntry||null):null;
    if(geminiContinuation){
      const expectedParent=String(geminiContinuation?.parentShadowTradeId||'');
      if(!anotherDimensionEntry?.id||anotherDimensionEntry.conceptName!=='Another Dimension'||String(anotherDimensionEntry.id)!==expectedParent||String(anotherDimensionEntry.ticker||'')!==String(q?.ticker||'')){
      ...
      if(String(anotherDimensionEntry.systemName||s.systemName)!==String(s.systemName)||String(anotherDimensionEntry.ownerId||s.ownerId)!==String(s.ownerId)||String(anotherDimensionEntry.mode||s.mode)!==String(s.mode)){
        await this.audit('athena_fire_execution_aborted',{boltId:bolt?.id||null,ticker:q?.ticker||null,concept,reason:'another_dimension_owner_or_mode_mismatch',stage:'structural_eligibility'});
        return null;
      }
      if((justiceArrowContinuation||athenaExclamationContinuation)&&(anotherDimensionEntry.status!=='closed'||Number(anotherDimensionEntry.remainingCount||0)>1e-9||String(anotherDimensionEntry.closeReason||'')!==ANOTHER_DIMENSION.profitableCloseReason||!(Number(anotherDimensionEntry.pnlCents||0)>0))){
        await this.audit('athena_fire_execution_aborted',{boltId:bolt?.id||null,ticker:q?.ticker||null,concept,reason:'another_dimension_profitable_close_required',stage:'structural_eligibility'});
        return null;
      }
      if(lightningPlasmaGemini){
        const fresh=lightningPlasmaStrikeQualification(anotherDimensionEntry,q,s,Date.now());
        if(!fresh.ok){
          await this.audit('athena_fire_execution_aborted',{boltId:bolt?.id||null,ticker:q?.ticker||null,concept,reason:`lightning_plasma_${fresh.reason}`,stage:'structural_eligibility'});
          return null;
        }
        lightningPlasmaGemini.upliftCents=Number(fresh.upliftCents||0);
        lightningPlasmaGemini.observedBidCents=Number(q.yesBid||0);
        lightningPlasmaGemini.observedAskCents=Number(q.yesAsk||0);
      }
      sourceTradeId=anotherDimensionEntry.id;
    }
    const entryQualificationSnapshot = {
      version:scarletContinuation?'SCARLET-CONTINUATION-Q1':justiceArrowContinuation?'JUSTICE-ARROW-CONTINUATION-Q1':athenaExclamationContinuation?'ATHENA-EXCLAMATION-AE2-Q1':lightningPlasmaGemini?'LIGHTNING-PLASMA-LP2-Q1':'ATHENA-A3-FIRE-Q1',boltId:bolt?.id||command.boltId,commandHash:command.commandHash,selectedAttack:concept,
      ...
      arayashiki:null,
      scarletNeedle:concept==='Scarlet Needle'?structuredClone(command?.decisionEvidence?.scarletNeedle||null):null,scarletContinuation,justiceArrowContinuation,athenaExclamationContinuation,lightningPlasmaGemini,observedAtMs:Date.now(),
    };
    const e = await this.createHunter(concept,q,Number(command.stakeCents),0,{
      sourceFeeder:geminiContinuation?'Another Dimension':source?.conceptName||null,sourceTradeId,sourceEntryConfig:geminiContinuation?anotherDimensionEntry?.entryConfig||null:source?.entryConfig||null,recoverySourceSnapshot,
  ...
  async lightningPlasmaCandidateTickers() {
    // LP1 ordinary-Cosmo discovery is retired. LP2 is evaluated only by the
    // Engine's Gemini/Another Dimension observer.
    return [];
  }

  async evaluateLightningPlasma(_marketMap, { legacyCompatibility=false } = {}) {
    if(legacyCompatibility===true)await this.audit('lightning_plasma_lp1_legacy_path_retired',{version:LIGHTNING_PLASMA.version});
    return [];
  }
