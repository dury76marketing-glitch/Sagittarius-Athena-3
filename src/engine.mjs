...
import { StrategyEngine, activeCosmoSources, lightningPlasmaFieldSelection, anotherDimensionQualification } from './strategy.mjs';
...
export class SagittariusEngine {
  constructor() {
    ...
    this.athenaExclamationStats = {version:ATHENA_EXCLAMATION.version,threshold:Number(ATHENA_EXCLAMATION.triggerEveryJusticeArrows||12),cycleState:null,offered:0,attempted:0,opened:0,fallbacks:0,fallbackJusticeOpened:0,blocked:0,lastEvent:null};
    this.lightningPlasmaStats = {version:LIGHTNING_PLASMA.version,evaluations:0,qualifiedMarkets:0,attempted:0,opened:0,slotBlocked:0,duplicateSuppressed:0,lastEvent:null};
    ...
    // R65/LP2 is a bounded continuous Gemini observer. Quote events only
    // coalesce one global evaluation; the durable three-slot lock and hard
    // execution railway remain authoritative, so no timing window grants entry.
  ...
  async handleScarletContinuation(parentEntry) {
    ...
    const profitCloseReason=String(parentEntry.closeReason||'');
    // R64 Scarlet continuation is authorized by a profitable universal
    // ATHENA-X1 close. Infinity Break remains accepted only for a parent exit
    // that was already committed under the pre-upgrade runtime.
    if(!['athena_x1_exit','infinity_break'].includes(profitCloseReason)||!(Number(parentEntry.pnlCents||0)>0))return {status:'IGNORED',reason:'not_profitable_authorized_profit_close'};
  ...
  observeAnotherDimensionQuote(q) {
    ...
    // LP2 is not a burst/window detector. Every fresh Gemini quote may make a
    // remembered Another Dimension market cross +4c, so coalesce one bounded
    // global field evaluation regardless of whether that AD trade is still
    // open or has already completed its virtual lifecycle.
    if(this.settings?.lightningPlasmaEnabled===true)this.queueLightningPlasmaEvaluation(ticker);
  ...
  justiceArrowRuntime() {
    ...
    if(!this.athenaExclamationStats||typeof this.athenaExclamationStats!=='object')this.athenaExclamationStats={version:ATHENA_EXCLAMATION.version,threshold:Number(ATHENA_EXCLAMATION.triggerEveryJusticeArrows||12),cycleState:null,offered:0,attempted:0,opened:0,fallbacks:0,fallbackJusticeOpened:0,blocked:0,lastEvent:null};
    return this.justiceArrowStats;
  }

  async currentAthenaExclamationCycleState() {
    const s=this.settings||{},threshold=Math.max(1,Number(ATHENA_EXCLAMATION.triggerEveryJusticeArrows||12));
    if(typeof this.db?.athenaExclamationCycleState==='function'){
      return this.db.athenaExclamationCycleState(s.systemName,ATHENA_EXCLAMATION.version,threshold,Number(s.resetTimestampMs||0));
    }
    // Test/compatibility fallback: reconstruct from the same frozen ledger
    // markers. Production Database implements the single-query path above.
    if(typeof this.db?.entriesByConcept!=='function')return null;
    const [aeRows,arrowRows]=await Promise.all([
      this.db.entriesByConcept(s.systemName,'Athena Exclamation',{limit:2000}).catch(()=>[]),
      this.db.entriesByConcept(s.systemName,'Sagittarius Justice Arrow',{limit:2000}).catch(()=>[]),
    ]);
    const reset=Math.max(0,Number(s.resetTimestampMs||0));
    const successfulStatuses=new Set(['open','exit_pending','pending_recovery','closed']);
    const ae2Rows=(aeRows||[]).filter((e)=>String(e.entryConfig?.athenaExclamation?.version||'')===ATHENA_EXCLAMATION.version&&Number(e.openedAtMs||0)>=reset);
    const latest=ae2Rows.filter((e)=>successfulStatuses.has(String(e.status||''))).sort((a,b)=>Number(b.openedAtMs||0)-Number(a.openedAtMs||0))[0]||null;
    const pending=ae2Rows.filter((e)=>String(e.status||'')==='entry_pending').sort((a,b)=>Number(b.openedAtMs||0)-Number(a.openedAtMs||0))[0]||null;
    const cycleStartedAtMs=Math.max(reset,Number(latest?.openedAtMs||0));
    const justiceArrowCount=(arrowRows||[]).filter((e)=>successfulStatuses.has(String(e.status||''))&&Number(e.openedAtMs||0)>cycleStartedAtMs&&String(e.entryConfig?.justiceArrowContinuation?.athenaExclamationCycleVersion||'')===ATHENA_EXCLAMATION.version).length;
    return{version:ATHENA_EXCLAMATION.version,threshold,justiceArrowCount,armed:justiceArrowCount>=threshold,cycleStartedAtMs,lastAthenaExclamationEntryId:latest?.id||null,pendingAthenaExclamationEntryId:pending?.id||null};
  }

  async handleJusticeArrowContinuation(parentShadow) {
    const stats=this.justiceArrowRuntime(),aeStats=this.athenaExclamationStats,s=this.settings||{};
    ...
    const recordAe=(status,data={})=>{aeStats.lastEvent={status,atMs:Date.now(),parentShadowTradeId:parentId,ticker,...data};return aeStats.lastEvent;};
    ...
    if(s.justiceArrowEnabled!==true&&s.athenaExclamationEnabled!==true)return{status:'IGNORED',reason:'gemini_real_continuations_disabled'};
    ...
    stats.eligible+=1;
    const routeKey=`GEMINI-CONTINUATION:${parentId}`;
    if(this.justiceArrowInFlight.has(routeKey)){stats.duplicateSuppressed+=1;record('DUPLICATE_SUPPRESSED',{scope:'local'});return{status:'DUPLICATE_SUPPRESSED',reason:'local_in_flight'};}
    this.justiceArrowInFlight.add(routeKey);
    let unlock=null;
    const justiceAuthorizationId=`JUSTICE-ARROW:${parentId}:1`,aeAuthorizationId=`ATHENA-EXCLAMATION:${parentId}:1`;
    const episode=(id,attack,decision={})=>({id,systemName:s.systemName,sourceRelease:RELEASE,cohortId:String(s.resetTimestampMs||''),ticker,eventTicker:String(parentShadow.eventTicker||ticker),side:'YES',sport:String(parentShadow.sport||'Unknown'),boltAtMs:Number(parentShadow.closedAtMs||Date.now()),boltSnapshot:{version:attack==='Athena Exclamation'?ATHENA_EXCLAMATION.version:SAGITTARIUS_JUSTICE_ARROW.version,parentShadowTradeId:parentId},athenaDecision:decision,fireCommand:{},attackSelected:attack,entryId:null,entryAtMs:null,outcome:{},trackingComplete:false,updatedAtMs:Date.now()});
    const justiceTerminal=async(reason,data={})=>{stats.blocked+=1;record('BLOCKED',{reason,authorizationId:justiceAuthorizationId,...data});const decision={decision:'BLOCKED',reason,strategicAuthority:false,justiceArrowContinuation:{version:SAGITTARIUS_JUSTICE_ARROW.version,status:'BLOCKED',terminal:true,authorizationId:justiceAuthorizationId,parentShadowTradeId:parentId,...data}};if(typeof this.db?.upsertOpportunityEpisode==='function')await this.db.upsertOpportunityEpisode(episode(justiceAuthorizationId,'Sagittarius Justice Arrow',decision)).catch(()=>{});await this.db.audit?.('info','justice_arrow_continuation_blocked',{authorizationId:justiceAuthorizationId,parentShadowTradeId:parentId,ticker,reason,...data}).catch(()=>{});return{status:'BLOCKED',reason,authorizationId:justiceAuthorizationId};};
    try{
      if(typeof this.db?.acquireHunterTickerLock!=='function')return justiceTerminal('continuation_lock_unavailable');
      // One cross-process router lock serializes both the cycle count and the
      // parent allocation. This prevents two profitable AD closes from both
      // observing arrow #12 and consuming the same Athena Exclamation turn.
      unlock=await this.db.acquireHunterTickerLock(s.systemName,'athena-exclamation-cycle-v2');
      if(!unlock){stats.duplicateSuppressed+=1;record('DUPLICATE_SUPPRESSED',{scope:'cycle_database_lock'});return{status:'DUPLICATE_SUPPRESSED',reason:'cycle_database_lock_busy'};}
      const [existingAe,existingJustice]=typeof this.db?.opportunityEpisode==='function'?await Promise.all([
        this.db.opportunityEpisode(aeAuthorizationId).catch(()=>null),this.db.opportunityEpisode(justiceAuthorizationId).catch(()=>null)
      ]):[null,null];
      const priorJusticeStatus=String(existingJustice?.athenaDecision?.justiceArrowContinuation?.status||existingJustice?.athenaDecision?.decision||'');
      if(existingAe?.entryId||existingJustice?.entryId||priorJusticeStatus==='BLOCKED'){stats.duplicateSuppressed+=1;record('DUPLICATE_SUPPRESSED',{scope:'durable_episode',entryId:existingAe?.entryId||existingJustice?.entryId||null,status:priorJusticeStatus||null});return{status:'DUPLICATE_SUPPRESSED',reason:'durable_authorization_already_consumed',entryId:existingAe?.entryId||existingJustice?.entryId||null};}
      const cycleState=await this.currentAthenaExclamationCycleState().catch(()=>null);
      if(!cycleState){aeStats.blocked+=1;recordAe('BLOCKED',{reason:'cycle_state_unavailable'});return justiceTerminal('athena_exclamation_cycle_state_unavailable');}
      aeStats.cycleState=structuredClone(cycleState);
      const refreshed=typeof this.market?.refreshTickerVerified==='function'?await this.market.refreshTickerVerified(ticker).catch(()=>null):null;
      if(!refreshed?.marketFresh||!refreshed?.bookFresh||!refreshed?.quote)return justiceTerminal('fresh_executable_market_unavailable');
      const q={...refreshed.quote,ticker,eventTicker:String(refreshed.quote.eventTicker||refreshed.quote.ticker||ticker)};
      const expectedEvent=String(parentShadow.eventTicker||ticker),status=String(q.status||'').toLowerCase();
      if(String(q.eventTicker||ticker)!==expectedEvent)return justiceTerminal('event_identity_mismatch',{expectedEventTicker:expectedEvent,freshEventTicker:q.eventTicker||null});
      if(status!=='active'||Boolean(q.result))return justiceTerminal('market_not_active',{status:status||null,result:q.result||null});

      // An unresolved LIVE AE intent is an ownership ambiguity, not a proven
      // fill. It does not reset the twelve-arrow cycle, but it also prevents a
      // second AE diversion until broker reconciliation resolves that intent.
      // Other Gemini opportunities remain eligible for ordinary Justice Arrow.
      const aeDiversionAvailable=s.athenaExclamationEnabled===true&&cycleState.armed===true&&!cycleState.pendingAthenaExclamationEntryId;
      if(aeDiversionAvailable){
        aeStats.offered+=1;aeStats.attempted+=1;recordAe('OFFERED',{authorizationId:aeAuthorizationId,justiceArrowCount:Number(cycleState.justiceArrowCount||0)});
        if(typeof this.db?.upsertOpportunityEpisode==='function')await this.db.upsertOpportunityEpisode(episode(aeAuthorizationId,'Athena Exclamation',{decision:'AUTHORIZED',reason:'post_12_justice_arrows_next_gemini_opportunity',strategicAuthority:false,athenaExclamationContinuation:{version:ATHENA_EXCLAMATION.version,status:'AUTHORIZED',terminal:false,authorizationId:aeAuthorizationId,parentShadowTradeId:parentId,justiceArrowCount:Number(cycleState.justiceArrowCount||0),cycleStartedAtMs:Number(cycleState.cycleStartedAtMs||0)}})).catch(()=>{});
        const aeOpened=await this.strategy.executeAthenaExclamationContinuation(q,parentShadow,{authorizationId:aeAuthorizationId,authorizedAtMs:Date.now(),cycleState});
        if(aeOpened){
          aeStats.opened+=1;recordAe('OPENED',{authorizationId:aeAuthorizationId,entryId:aeOpened.id,entryPriceCents:Number(aeOpened.entryPriceCents||0)});
          const openedDecision={decision:'OPENED',reason:'athena_exclamation_diversion_opened',strategicAuthority:false,athenaExclamationContinuation:{version:ATHENA_EXCLAMATION.version,status:'OPENED',terminal:true,authorizationId:aeAuthorizationId,parentShadowTradeId:parentId,justiceArrowCount:Number(cycleState.justiceArrowCount||0),entryId:aeOpened.id,entryAtMs:Number(aeOpened.openedAtMs||Date.now())}};
          const current=typeof this.db?.opportunityEpisode==='function'?await this.db.opportunityEpisode(aeAuthorizationId).catch(()=>null):null;
          if(typeof this.db?.upsertOpportunityEpisode==='function')await this.db.upsertOpportunityEpisode({...episode(aeAuthorizationId,'Athena Exclamation',openedDecision),...(current||{}),athenaDecision:openedDecision,fireCommand:structuredClone(aeOpened.entryConfig?.athenaFire||current?.fireCommand||{}),entryId:aeOpened.id,entryAtMs:Number(aeOpened.openedAtMs||Date.now()),attackSelected:'Athena Exclamation',updatedAtMs:Date.now()}).catch(()=>{});
          ...
          if(parentDurable&&typeof this.db?.updateEntry==='function'){
            const feederState={...(parentDurable.feederState||parentShadow.feederState||{}),universe:'Gemini',athenaExclamationEntryId:aeOpened.id,athenaExclamationOpenedAtMs:Number(aeOpened.openedAtMs||Date.now()),athenaExclamationAuthorizationId:aeAuthorizationId};
            await this.db.updateEntry(parentId,{feederState,updatedAtMs:Date.now()}).catch(()=>{});this.rememberAnotherDimension({...parentDurable,feederState,updatedAtMs:Date.now()});
          }
          await this.db.audit?.('info','athena_exclamation_diversion_opened',{authorizationId:aeAuthorizationId,parentShadowTradeId:parentId,ticker,entryId:aeOpened.id,justiceArrowCount:Number(cycleState.justiceArrowCount||0)}).catch(()=>{});
          return{status:'OPENED',attack:'Athena Exclamation',authorizationId:aeAuthorizationId,entry:aeOpened};
        }
        // Failed AE execution never consumes the profitable Gemini opportunity
        // and never resets the twelve-arrow cycle. Justice Arrow receives the
        // exact same still-valid quote through its normal hard-safety path.
        aeStats.fallbacks+=1;recordAe('FALLBACK',{authorizationId:aeAuthorizationId,reason:'hard_safety_or_execution_blocked'});
        const fallbackDecision={decision:'FALLBACK',reason:'athena_exclamation_unfilled_fallback_to_justice_arrow',strategicAuthority:false,athenaExclamationContinuation:{version:ATHENA_EXCLAMATION.version,status:'FALLBACK',terminal:true,authorizationId:aeAuthorizationId,parentShadowTradeId:parentId,justiceArrowCount:Number(cycleState.justiceArrowCount||0)}};
        if(typeof this.db?.upsertOpportunityEpisode==='function')await this.db.upsertOpportunityEpisode({...episode(aeAuthorizationId,'Athena Exclamation',fallbackDecision),athenaDecision:fallbackDecision,trackingComplete:true,updatedAtMs:Date.now()}).catch(()=>{});
        await this.db.audit?.('info','athena_exclamation_fallback_to_justice_arrow',{authorizationId:aeAuthorizationId,parentShadowTradeId:parentId,ticker,justiceArrowCount:Number(cycleState.justiceArrowCount||0)}).catch(()=>{});
      }

      if(s.justiceArrowEnabled!==true)return justiceTerminal('justice_arrow_disabled_after_athena_exclamation_fallback');
      const existingStatus=String(existingJustice?.athenaDecision?.justiceArrowContinuation?.status||existingJustice?.athenaDecision?.decision||'');
      if(['OPENED','BLOCKED'].includes(existingStatus)){stats.duplicateSuppressed+=1;return{status:'DUPLICATE_SUPPRESSED',reason:'justice_arrow_authorization_already_consumed',entryId:existingJustice?.entryId||null};}
      const authorizedAtMs=Date.now();
      const lineage={version:SAGITTARIUS_JUSTICE_ARROW.version,status:'AUTHORIZED',terminal:false,authorizationId:justiceAuthorizationId,parentShadowTradeId:parentId,parentCloseReason:String(parentShadow.closeReason||''),parentRealizedPnlCents:Number(parentShadow.pnlCents||0),parentExitPriceCents:Number(parentShadow.exitPriceCents||0),parentClosedAtMs:Number(parentShadow.closedAtMs||0),sourceCosmo:String(parentShadow.sourceFeeder||''),sourceCosmoTradeId:String(parentShadow.sourceTradeId||''),authorizedAtMs,athenaExclamationCycleVersion:ATHENA_EXCLAMATION.version,fullExecutionSafetyRequired:true,normalStrategicDiscoveryBypassed:true,profitAuthority:ATHENA_EXIT_INTELLIGENCE.version};
      if(typeof this.db?.upsertOpportunityEpisode==='function')await this.db.upsertOpportunityEpisode(episode(justiceAuthorizationId,'Sagittarius Justice Arrow',{decision:'AUTHORIZED',reason:'profitable_another_dimension_close_authorized_justice_arrow',strategicAuthority:false,justiceArrowContinuation:lineage})).catch(()=>{});
      stats.authorized+=1;stats.attempted+=1;record('ATTEMPTED',{authorizationId:justiceAuthorizationId,bidCents:Number(q.yesBid||0),askCents:Number(q.yesAsk||0),athenaExclamationFallback:aeDiversionAvailable,athenaExclamationPendingBypass:Boolean(cycleState.pendingAthenaExclamationEntryId)});
      const opened=await this.strategy.executeJusticeArrowContinuation(q,parentShadow,{authorizationId:justiceAuthorizationId,authorizedAtMs});
      if(!opened)return justiceTerminal('hard_safety_or_execution_blocked');
      stats.opened+=1;if(aeDiversionAvailable)aeStats.fallbackJusticeOpened+=1;record('OPENED',{authorizationId:justiceAuthorizationId,entryId:opened.id,entryPriceCents:Number(opened.entryPriceCents||0)});
      const parentDurable=typeof this.db?.entryById==='function'?await this.db.entryById(parentId).catch(()=>null):parentShadow;
      if(parentDurable&&typeof this.db?.updateEntry==='function'){
        const feederState={...(parentDurable.feederState||parentShadow.feederState||{}),universe:'Gemini',justiceArrowEntryId:opened.id,justiceArrowOpenedAtMs:Number(opened.openedAtMs||Date.now()),justiceArrowAuthorizationId:justiceAuthorizationId};
        await this.db.updateEntry(parentId,{feederState,updatedAtMs:Date.now()}).catch(()=>{});this.rememberAnotherDimension({...parentDurable,feederState,updatedAtMs:Date.now()});
      }
      ...
      const current=typeof this.db?.opportunityEpisode==='function'?await this.db.opportunityEpisode(justiceAuthorizationId).catch(()=>null):null;
      if(typeof this.db?.upsertOpportunityEpisode==='function')await this.db.upsertOpportunityEpisode({...episode(justiceAuthorizationId,'Sagittarius Justice Arrow',openedDecision),...(current||{}),athenaDecision:openedDecision,fireCommand:structuredClone(opened.entryConfig?.athenaFire||current?.fireCommand||{}),entryId:opened.id,entryAtMs:Number(opened.openedAtMs||Date.now()),attackSelected:'Sagittarius Justice Arrow',updatedAtMs:Date.now()}).catch(()=>{});
      await this.db.audit?.('info','justice_arrow_continuation_opened',{authorizationId:justiceAuthorizationId,parentShadowTradeId:parentId,ticker,entryId:opened.id,entryPriceCents:Number(opened.entryPriceCents||0),profitAuthority:opened.entryConfig?.profitAuthority||null,athenaExclamationFallback:aeDiversionAvailable,athenaExclamationPendingBypass:Boolean(cycleState.pendingAthenaExclamationEntryId)}).catch(()=>{});
      return{status:'OPENED',attack:'Sagittarius Justice Arrow',authorizationId:justiceAuthorizationId,entry:opened};
    }catch(error){await this.db.audit?.('error','gemini_real_continuation_failed',{parentShadowTradeId:parentId,ticker,message:String(error?.message||error)}).catch(()=>{});return justiceTerminal('continuation_exception',{message:String(error?.message||error)});}
    finally{if(unlock)await unlock().catch(()=>{});this.justiceArrowInFlight.delete(routeKey);}
  }
  ...
  async init() {
    ...
    this.profitGuard = new ProfitGuard({
      ...
      onOpportunityCompleted: (episode) => this.athenaCommander?.learnEpisode?.(episode),
      onPositionClosed: (entry) => this.queueScarletContinuation(entry),
    });
  ...
  async evaluateNewGenerationOpportunities(marketMap,{onlyTicker=null}={}) {
    ...
    for(const original of candidates){
      ...
      // R65: Lightning Plasma and Athena Exclamation are no longer ordinary
      // Bolt/Athena candidates. Their Gemini-direct routers are independent of
      // this field context, which stays explicitly non-authoritative here.
      const preliminaryFieldContext={version:'R65-GEMINI-DIRECT-ATTACKS',lightningPlasmaQualified:false,currentTickerEligible:false,currentEventEligible:false,sourceCount:0,cosmoCount:cosmos.length,independentEventCount:0,minCosmos:0,minStrikes:0,maxRays:0,rayStakeCents:0,fieldBudgetCents:0,expiresAtMs:0,sourceCosmoId:null,athenaExclamationCandidate:null,goldSaintCount:0};
  ...
  async runGoldenEyeEvaluation(source = 'quote_event') {
    ...
    this.goldenEyeEvaluationPromise = (async () => {
      ...
      const goldenEyeAuthorityEntryIds=entries
        .filter((e)=>!ACTIVE_PORTFOLIO_CONCEPTS.has(String(e?.conceptName||''))&&String(e?.entryConfig?.profitAuthority||'')===GOLDEN_EYE.version)
        .map((e)=>String(e.id||'')).filter(Boolean);
      const hasFrozenGoldenEye = goldenEyeAuthorityEntryIds.length>0;
      ...
      const simulationAllowed = this.settings.mode === 'SIMULATION' && hasFrozenGoldenEye;
      ...
      this.goldenEyeExecutionPromise = (async () => {
        const result = await this.manualCashout({ entryIds:goldenEyeAuthorityEntryIds }, 'golden_eye_cashout');
  ...
  activeAthenaExclamationCandidate(ticker,now=Date.now()){
    void ticker;void now;
    // AE1 three-Saint candidates are retired in R65.
    return null;
  }
  ...
  queueLightningPlasmaEvaluation(ticker=null) {
    if (!this.running || !this.strategy || this.settings.lightningPlasmaEnabled !== true || this.settings.geminiEnabled !== true) return false;
    if (!this.entryExecutionGate().allowed) return false;
    if(this.lightningPlasmaEvaluationPromise){this.lightningPlasmaRerunRequested=true;return true;}
    if(this.lightningPlasmaTimer){this.lightningPlasmaRerunRequested=true;return true;}
    this.lightningPlasmaTimer=setTimeout(()=>{
      this.lightningPlasmaTimer=null;
      void this.runLightningPlasmaEvaluation(ticker?'quote_event':'runtime').catch(async(error)=>{
        await this.db.audit?.('error','lightning_plasma_gemini_evaluation',{ticker:ticker||null,message:String(error?.message||error)}).catch(()=>{});
      });
    },100);
    return true;
  }

  async runLightningPlasmaEvaluation(source='quote_event') {
    if(this.lightningPlasmaEvaluationPromise){this.lightningPlasmaRerunRequested=true;return this.lightningPlasmaEvaluationPromise;}
    if(!this.running||!this.strategy||this.settings.lightningPlasmaEnabled!==true||this.settings.geminiEnabled!==true||!this.entryExecutionGate().allowed)return null;
    this.lightningPlasmaEvaluationPromise=(async()=>{
      const s=this.settings,stats=this.lightningPlasmaStats||(this.lightningPlasmaStats={version:LIGHTNING_PLASMA.version,evaluations:0,qualifiedMarkets:0,attempted:0,opened:0,slotBlocked:0,duplicateSuppressed:0,lastEvent:null});
      stats.evaluations+=1;
      let unlock=null;
      try{
        if(typeof this.db?.acquireHunterTickerLock!=='function'){stats.lastEvent={status:'BLOCKED',reason:'slot_lock_unavailable',atMs:Date.now(),source};return null;}
        unlock=await this.db.acquireHunterTickerLock(s.systemName,'lightning-plasma-slots-v2');
        if(!unlock){stats.duplicateSuppressed+=1;stats.lastEvent={status:'COALESCED',reason:'slot_lock_busy',atMs:Date.now(),source};return null;}
        const lpRows=typeof this.db?.entriesByConcept==='function'?await this.db.entriesByConcept(s.systemName,'Lightning Plasma',{limit:2000}).catch(()=>[]):[];
        const active=(lpRows||[]).filter((e)=>openLike(e.status));
        const maxSlots=Number(LIGHTNING_PLASMA.maxStrikes);
        let available=Math.max(0,maxSlots-active.length);
        if(available<=0){stats.slotBlocked+=1;stats.lastEvent={status:'FULL',atMs:Date.now(),source,active:active.length,maxSlots};return{status:'FULL',active:active.length,maxSlots};}
        const refs=[...(this.anotherDimensionRecent?.values?.()||[])].filter((e)=>e?.id&&e.conceptName==='Another Dimension'&&String(e.systemName||'')===String(s.systemName)&&String(e.ownerId||'')===String(s.ownerId)&&String(e.mode||'')===String(s.mode)&&String(e.status||'')!=='rejected');
        const marketMap=new Map();
        for(const ref of refs){const q=this.market?.getQuote?.(String(ref.ticker||''));if(q)marketMap.set(String(ref.ticker),q);}
        const field=lightningPlasmaFieldSelection(refs,marketMap,s,Date.now());
        stats.qualifiedMarkets=Number(field.independentMarketCount||0);
        const usedSourceIds=new Set((lpRows||[]).filter((e)=>e.status!=='rejected'&&e.sourceTradeId).map((e)=>String(e.sourceTradeId)));
        const activeTickers=new Set(active.map((e)=>String(e.ticker||'')).filter(Boolean));
        const configuredBudget=Math.max(1,Math.floor(Number(s.lightningPlasmaFieldStakeCents||LIGHTNING_PLASMA.fieldStakeCents)));
        const rayStakeCents=Math.max(1,Math.floor(configuredBudget/maxSlots));
        const opened=[];
        for(const candidate of field.candidates||[]){
          if(available<=0)break;
          const sourceId=String(candidate?.source?.id||''),ticker=String(candidate?.ticker||candidate?.source?.ticker||'');
          if(!sourceId||!ticker||usedSourceIds.has(sourceId)||activeTickers.has(ticker))continue;
          const durable=typeof this.db?.entryById==='function'?await this.db.entryById(sourceId).catch(()=>null):candidate.source;
          if(!durable||durable.conceptName!=='Another Dimension'||String(durable.systemName||'')!==String(s.systemName)||String(durable.ownerId||'')!==String(s.ownerId)||String(durable.mode||'')!==String(s.mode)||String(durable.status||'')==='rejected')continue;
          const refreshed=typeof this.market?.refreshTickerVerified==='function'?await this.market.refreshTickerVerified(ticker).catch(()=>null):null;
          if(!refreshed?.marketFresh||!refreshed?.bookFresh||!refreshed?.quote)continue;
          const q={...refreshed.quote,ticker,eventTicker:String(refreshed.quote.eventTicker||refreshed.quote.ticker||ticker)};
          const expectedEvent=String(durable.eventTicker||ticker),status=String(q.status||'').toLowerCase();
          if(String(q.eventTicker||ticker)!==expectedEvent||status!=='active'||Boolean(q.result))continue;
          const qualification=lightningPlasmaFieldSelection([durable],new Map([[ticker,q]]),s,Date.now()).candidates?.[0]?.qualification;
          if(!qualification?.ok)continue;
          if(typeof this.db?.entryByConceptSourceTradeId==='function'){
            const existing=await this.db.entryByConceptSourceTradeId(s.systemName,'Lightning Plasma',sourceId).catch(()=>null);
            if(existing&&existing.status!=='rejected'){usedSourceIds.add(sourceId);stats.duplicateSuppressed+=1;continue;}
          }
          const authorizationId=`LIGHTNING-PLASMA:${sourceId}:4C`;
          stats.attempted+=1;
          const entry=await this.strategy.executeLightningPlasmaGeminiStrike(q,durable,{authorizationId,authorizedAtMs:Date.now(),slotIndex:active.length+opened.length+1,maxSlots,rayStakeCents,fieldBudgetCents:configuredBudget});
          if(!entry)continue;
          opened.push(entry);stats.opened+=1;available-=1;usedSourceIds.add(sourceId);activeTickers.add(ticker);
          stats.lastEvent={status:'OPENED',atMs:Date.now(),source,entryId:entry.id,parentShadowTradeId:sourceId,ticker,upliftCents:Number(qualification.upliftCents||0),slotIndex:active.length+opened.length,maxSlots,fieldBudgetCents:configuredBudget,rayStakeCents};
          await this.db.audit?.('info','lightning_plasma_gemini_strike_opened',{entryId:entry.id,parentShadowTradeId:sourceId,ticker,upliftCents:Number(qualification.upliftCents||0),activeBefore:active.length,openedThisRun:opened.length,maxSlots,fieldBudgetCents:configuredBudget,rayStakeCents,noTimeWindow:true}).catch(()=>{});
        }
        if(!opened.length)stats.lastEvent={status:'OBSERVED',atMs:Date.now(),source,qualifiedMarkets:Number(field.independentMarketCount||0),availableSlots:available,maxSlots};
        return{status:opened.length?'OPENED':'OBSERVED',opened,qualifiedMarkets:Number(field.independentMarketCount||0),availableSlots:available,maxSlots};
      }finally{if(unlock)await unlock().catch(()=>{});}
    })();
    try{return await this.lightningPlasmaEvaluationPromise;}
    finally{
      this.lightningPlasmaEvaluationPromise=null;
      if(this.lightningPlasmaRerunRequested){this.lightningPlasmaRerunRequested=false;this.queueLightningPlasmaEvaluation();}
    }
  }
  ...
  async applySettingsPatch(patch) {
    ...
    if(Number(next.lightningPlasmaMaxStrikes)!==Number(LIGHTNING_PLASMA.maxStrikes))throw new Error('lightningPlasmaMaxStrikes is fixed at 3 by LP2 doctrine');
  ...
  buildInfinityBreakSummary(entries=[]){
    ...
    return{version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,role:'LEGACY_REAL_EXIT_COMPATIBILITY_AND_X1_THRESHOLD_SETTINGS',authority:'NO_NEW_REAL_ATTACK_EXIT_AUTHORITY',legacyCompatibilityOnly:true,positionsObserved:rows.length,activePositions:active.length,breaksExecuted:closed.length,realizedPnlCents:pnl,averageProfitCents:closed.length?pnl/closed.length:0,averageTimeToBreakMs:closed.length?closed.reduce((sum,e)=>sum+Math.max(0,Number(e.closedAtMs||0)-Number(e.openedAtMs||0)),0)/closed.length:0,minimumNetPerOriginalContractCents:Number(this.settings.infinityBreakMinNetPerOriginalContractCents??INFINITY_BREAK.defaultMinimumNetPerOriginalContractCents),minimumNetRole:'ATHENA_X1_ACTIVATION_THRESHOLD_FOR_NEW_REAL_ATTACKS',requiredFreshConfirmations:Number(this.settings.infinityBreakRequiredConfirmations??INFINITY_BREAK.defaultRequiredFreshConfirmations),maximumBookAgeMs:Number(this.settings.infinityBreakMaximumBookAgeMs??INFINITY_BREAK.defaultMaximumBookAgeMs),confirmationWindowMs:Number(this.settings.infinityBreakConfirmationWindowMs??INFINITY_BREAK.defaultConfirmationWindowMs),fullPositionOnly:true,lossAuthority:INFINITY_BREAK.lossAuthority};
  }

  buildAthenaX1Summary(entries=[]){
    const rows=(entries||[]).filter((e)=>ACTIVE_PORTFOLIO_CONCEPTS.has(String(e?.conceptName||''))&&String(e?.entryConfig?.profitAuthority||'')===ATHENA_EXIT_INTELLIGENCE.version);
    const closed=rows.filter((e)=>e.status==='closed'&&String(e.closeReason||'')==='athena_x1_exit');
    const active=rows.filter((e)=>openLike(e.status));
    const activated=rows.filter((e)=>Number(e?.profitGuardState?.activationReachedAtMs||0)>0&&e?.profitGuardState?.activationLatched===true);
    const pnl=closed.reduce((sum,e)=>sum+Number(e.pnlCents||0),0);
    return{
      version:ATHENA_EXIT_INTELLIGENCE.version,policyRevision:ATHENA_EXIT_INTELLIGENCE.policyRevision,
      role:ATHENA_EXIT_INTELLIGENCE.role,authority:'UNIVERSAL_REAL_EXECUTION_ATTACK_PROFIT_AUTHORITY',
      positionsObserved:rows.length,activePositions:active.length,activatedPositions:activated.length,
      waitingForActivation:active.filter((e)=>!(Number(e?.profitGuardState?.activationReachedAtMs||0)>0&&e?.profitGuardState?.activationLatched===true)).length,
      exitsExecuted:closed.length,realizedPnlCents:pnl,averageProfitCents:closed.length?pnl/closed.length:0,
      averageTimeToExitMs:closed.length?closed.reduce((sum,e)=>sum+Math.max(0,Number(e.closedAtMs||0)-Number(e.openedAtMs||0)),0)/closed.length:0,
      activationThresholdSetting:'infinityBreakMinNetPerOriginalContractCents',
      activationMinimumNetPerOriginalContractCents:Number(this.settings.infinityBreakMinNetPerOriginalContractCents??INFINITY_BREAK.defaultMinimumNetPerOriginalContractCents),
      activationRequiresFreshFullPositionExecutableDepth:true,activationLatch:true,fullPositionOnly:true,
      lossAuthority:AURORA_EXECUTION.version,anotherDimensionExcluded:true,
    };
  }
  ...
  async collectState() {
    ...
    const geminiDirectExposureEnabled=this.settings.geminiEnabled===true&&(this.settings.justiceArrowEnabled===true||this.settings.lightningPlasmaEnabled===true||this.settings.athenaExclamationEnabled===true);
    if (!initialExposureEnabled.length&&!geminiDirectExposureEnabled) entryPathWarnings.push('No currently enabled configuration can create a real Execution Attack exposure');
    ...
    const entryPathConfiguration = {
      version:'EPC13-R65', authorityChain:'GAME_CLOCK->COSMO_SHADOW->COSMO_GREEN->ATOMIC_THUNDER_BOLT->ATHENA->ORDINARY_ATTACK->ATHENA_X1/AURORA + PROFITABLE_ATHENA_X1_CLOSE->SCARLET_NEEDLE->HARD_SAFETY->ATHENA_X1/AURORA + COSMO_SHADOW->GEMINI->ANOTHER_DIMENSION->{PLUS_4C->LIGHTNING_PLASMA[3_SLOTS], PROFITABLE_CLOSE->{EVERY_12_JUSTICE_ARROWS_NEXT->ATHENA_EXCLAMATION_ELSE_SAGITTARIUS_JUSTICE_ARROW}}->HARD_SAFETY->ATHENA_X1/AURORA', noBoltNoAttack:true, noBoltNoAttackExceptions:['SCARLET_NEEDLE_POST_PROFIT_CONTINUATION','SAGITTARIUS_JUSTICE_ARROW_POST_SHADOW_WIN','ATHENA_EXCLAMATION_POST_12_JUSTICE_ARROWS','LIGHTNING_PLASMA_GEMINI_PLUS_4C'], cosmoGreenRequired:true, initialExposureEnabled, geminiDirectExposureEnabled, scarletNeedleContinuationEnabled:this.settings.scarletNeedleEnabled===true&&Number(this.settings.scarletNeedleMaxRepeats||0)>0, geminiEnabled:this.settings.geminiEnabled===true, anotherDimensionAttackEnabled:this.settings.geminiEnabled===true, justiceArrowContinuationEnabled:this.settings.justiceArrowEnabled===true, athenaExclamationDiversionEnabled:this.settings.athenaExclamationEnabled===true, lightningPlasmaGeminiEnabled:this.settings.lightningPlasmaEnabled===true, lightningPlasmaMinimumUpliftCents:Number(LIGHTNING_PLASMA.minimumUpliftCents),lightningPlasmaFixedSlots:Number(LIGHTNING_PLASMA.maxStrikes),athenaExclamationEveryJusticeArrows:Number(ATHENA_EXCLAMATION.triggerEveryJusticeArrows), recoveryFollowOnEnabled:this.settings.recoveryHunterEnabled === true, warnings:entryPathWarnings,
    ...
    const athenaX1 = this.buildAthenaX1Summary(entries);
    ...
    return {
      ...
      riskControls: {
        ...
        authorityChain:'GAME_CLOCK->COSMO_SHADOW->COSMO_GREEN->ATOMIC_THUNDER_BOLT->ATHENA->EXECUTION_ATTACK->ATHENA_X1/AURORA',
        noBoltNoAttack:true,
        ...
        noBoltNoAttackExceptions:['SCARLET_NEEDLE_POST_PROFIT_CONTINUATION','SAGITTARIUS_JUSTICE_ARROW_POST_SHADOW_WIN','ATHENA_EXCLAMATION_POST_12_JUSTICE_ARROWS','LIGHTNING_PLASMA_GEMINI_PLUS_4C'],
        ...
        athenaExclamationRole:ATHENA_EXCLAMATION.role,
        athenaExclamationStrategicAuthority:ATHENA_EXCLAMATION.strategicEntryAuthority,
        athenaExclamationEveryJusticeArrows:Number(ATHENA_EXCLAMATION.triggerEveryJusticeArrows),
        athenaExclamationFallbackAttack:ATHENA_EXCLAMATION.fallbackAttack,
        athenaExclamationCycle:{...(this.athenaExclamationStats||{})},
        ...
        lightningPlasmaSourceUniverse:LIGHTNING_PLASMA.sourceUniverse,
        lightningPlasmaSourceAttack:LIGHTNING_PLASMA.sourceAttack,
        lightningPlasmaMinimumUpliftCents:Number(LIGHTNING_PLASMA.minimumUpliftCents),
        lightningPlasmaFixedSlots:Number(LIGHTNING_PLASMA.maxStrikes),
        lightningPlasmaNoTimeWindow:LIGHTNING_PLASMA.noTimeWindow===true,
        lightningPlasmaRuntime:{...(this.lightningPlasmaStats||{})},
        lightningPlasmaFieldBudgetSharedAcrossStrikes:true,
        lightningPlasmaOneStrikePerTicker:true,
        ...
        athenaConsumers: [...ACTIVE_PORTFOLIO_CONCEPTS],
        ...
        athenaExitAppliesTo: 'R64_universal_all_current_real_Execution_Attacks',
        athenaExitActivationThresholdSetting: ATHENA_EXIT_INTELLIGENCE.activationThresholdSetting,
        athenaExitActivationMinimumNetPerOriginalContractCents: Number(this.settings.infinityBreakMinNetPerOriginalContractCents??INFINITY_BREAK.defaultMinimumNetPerOriginalContractCents),
        athenaExitActivationRequiresFreshFullPositionExecutableDepth: ATHENA_EXIT_INTELLIGENCE.activationRequiresFreshFullPositionExecutableDepth,
        athenaExitActivationLatch: ATHENA_EXIT_INTELLIGENCE.activationLatch,
        athenaExitAnotherDimensionExcluded: true,
        ...
        goldenEyeAppliesTo: 'legacy_non_current_creation_time_GOLDEN_EYE_positions_only; current real Execution Attacks are ATHENA-X1',
