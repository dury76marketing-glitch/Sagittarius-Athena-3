...
export class AthenaExclamationEngine {
  constructor({db=null,getSettings=()=>({}),audit=null}={}){
    ...
    this.counters={votesRecorded:0,votesRefreshed:0,formations:0,qualified:0,rejected:0,executed:0};
  }
  ...
  async init(now=Date.now()){
    ...
    if(typeof this.db?.recentAthenaExclamationEvents==='function'){
      ...
      for(const e of rows||[]){
        ...
        if(finite(e.expiresAtMs,0)>now)this.activeEventByTicker.set(String(e.ticker),String(e.id));
      }
    }
  ...
  prune(now=Date.now()){
    ...
    for(const [ticker,id] of this.activeEventByTicker){const e=this.events.get(id);if(!e||finite(e.expiresAtMs,0)<=now)this.activeEventByTicker.delete(ticker);}
  ...
  async recordQualification({conceptName,ticker,eventTicker=null,qualifiedAtMs=Date.now(),priceCents=0,bidCents=0,sourceFeeder=null,sourceTradeId=null,gameMinutes=null,qualificationSnapshot=null}={}){
    ...
    if(typeof this.db?.acquireHunterTickerLock==='function'){
      ...
      if(!formationUnlock)return {recorded:true,vote,candidate:null,reason:'formation_lock_busy'};
    }
      ...
      if(activeId){
        ...
        if(active){
          ...
          if(!already){
            ...
            if(typeof this.db?.updateAthenaExclamationEvent==='function')await this.db.updateAthenaExclamationEvent(activeId,updated).catch(()=>{});
  ...
  async markDecision(id,{status,review=null,entryId=null,decidedAtMs=Date.now()}={}){
    ...
    if(typeof this.db?.updateAthenaExclamationEvent==='function')await this.db.updateAthenaExclamationEvent(String(id),next).catch(()=>{});
    return next;
  }
  summary(now=Date.now()){
    this.prune(now);
    const byTicker={};for(const v of this.votes.values()){const k=String(v.ticker);byTicker[k]=(byTicker[k]||0)+1;}
    return {version:ATHENA_EXCLAMATION.version,policyRevision:ATHENA_EXCLAMATION.policyRevision,minimumSaints:ATHENA_EXCLAMATION.minimumSaints,convergenceWindowMinutes:athenaExclamationWindowMs(this.settings())/60000,activeVotes:this.votes.size,activeTickers:Object.keys(byTicker).length,activeFormations:this.activeEventByTicker.size,...this.counters,recentEvents:[...this.events.values()].sort((a,b)=>finite(b.createdAtMs)-finite(a.createdAtMs)).slice(0,25)};
  }
}
