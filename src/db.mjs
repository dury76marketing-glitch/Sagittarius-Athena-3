...
export class Database {
  ...
  async athenaExclamationCycleState(systemName,cycleVersion,threshold=12,resetTimestampMs=0){
    const system=String(systemName),version=String(cycleVersion),needed=Math.max(1,Math.floor(Number(threshold)||12)),reset=Math.max(0,Number(resetTimestampMs)||0);
    // AE2 deliberately reconstructs its counter from the real durable entry
    // ledger. There is no mutable in-memory counter that can double-increment
    // on retry or reset on a Railway restart. Only entries with a proven fill
    // state count: unresolved entry_pending intents cannot arm/reset the cycle.
    // The latest successfully opened AE2 entry is the cycle boundary; only
    // successfully opened Justice Arrows after that boundary count to twelve.
    const r=await this.pool.query(`with last_ae as (
      select id,opened_at_ms from sag_entries
       where system_name=$1 and archived=false and concept_name='Athena Exclamation'
         and status in ('open','exit_pending','pending_recovery','closed')
         and coalesce(entry_config#>>'{athenaExclamation,version}','')=$2 and opened_at_ms >= $3
       order by opened_at_ms desc,id desc limit 1
    ), pending_ae as (
      select id from sag_entries
       where system_name=$1 and archived=false and concept_name='Athena Exclamation' and status='entry_pending'
         and coalesce(entry_config#>>'{athenaExclamation,version}','')=$2 and opened_at_ms >= $3
       order by opened_at_ms desc,id desc limit 1
    ), boundary as (
      select greatest($3::bigint,coalesce((select opened_at_ms from last_ae),0)::bigint) as cycle_started_at_ms
    ), arrows as (
      select count(*)::int as justice_arrow_count from sag_entries,boundary
       where system_name=$1 and archived=false and concept_name='Sagittarius Justice Arrow'
         and status in ('open','exit_pending','pending_recovery','closed')
         and coalesce(entry_config#>>'{justiceArrowContinuation,athenaExclamationCycleVersion}','')=$2
         and opened_at_ms > boundary.cycle_started_at_ms
    ) select boundary.cycle_started_at_ms,arrows.justice_arrow_count,
      (select id from last_ae) as last_athena_exclamation_entry_id,
      (select id from pending_ae) as pending_athena_exclamation_entry_id
      from boundary,arrows`,[system,version,reset]);
    const row=r.rows?.[0]||{},count=Math.max(0,Number(row.justice_arrow_count)||0),cycleStartedAtMs=Math.max(reset,Number(row.cycle_started_at_ms)||0);
    return{version,threshold:needed,justiceArrowCount:count,armed:count>=needed,cycleStartedAtMs,lastAthenaExclamationEntryId:row.last_athena_exclamation_entry_id||null,pendingAthenaExclamationEntryId:row.pending_athena_exclamation_entry_id||null};
  }
