export const GAME_CLOCK_AUTHORITY = Object.freeze({
  ...
  simulationActivityMaxGapMs: 10 * 60 * 1000,
  simulationActivityMinimumObservations: 2,
...
function stateBase(eventTicker, now) {
  return {
    ...
    simulationActivityStartMs: null,
    simulationActivityLastEvidenceMs: null,
    simulationActivityEvidenceCount: 0,
...
function makeConfirmed(eventTicker, now, startTimeMs, source, extra = {}) {
  ...
  return {
    ...
    sourceStrength: source === 'occurrence_passed'
      ? 'fallback'
      : source === 'simulation_observed_activity'
        ? 'simulation_only'
        : 'strong',
...
export class GameClockAuthority {
  ...
  async resolveEvent({
    ...
    allowGameStats = true,
    forceFresh = false,
    allowSimulationActivityClock = false,
    minimumElapsedMs = 0,
  } = {}) {
    ...
    if (prior?.confirmed) {
      if (prior.source === 'simulation_observed_activity') {
        const freshSimulationActivity = Boolean(allowSimulationActivityClock && broadLive(quotes) && freshTradeActivity(quotes, currentNow));
        const minimumAge = Math.max(0, Number(minimumElapsedMs) || 0);
        const age = currentNow - Number(prior.startTimeMs || 0);
        if (freshSimulationActivity && age >= minimumAge) {
          return makeConfirmed(event, currentNow, prior.startTimeMs, 'simulation_observed_activity', {
            ...milestoneMeta,
            occurrenceTimeMs,
            ...
            authorizationReason: 'simulation_observed_activity_aged_plus_fresh_exact_trade',
            authorizationSource: 'simulation_observed_activity_fresh_exact_trade',
            reason: 'simulation_observed_activity_reauthorized',
            simulationActivityStartMs: Number(prior.simulationActivityStartMs || prior.startTimeMs),
            simulationActivityLastEvidenceMs: currentNow,
            simulationActivityEvidenceCount: Math.max(GAME_CLOCK_AUTHORITY.simulationActivityMinimumObservations, Number(prior.simulationActivityEvidenceCount || 0) + 1),
            evidence: { ...(prior.evidence || {}), simulationOnly:true, freshTradeActivity:true, minimumElapsedMs:minimumAge, observedAgeMs:age },
          });
        }
        return {
          ...prior,
          ...
          authorizationSource:null,
          lastCheckedAtMs:currentNow,
          reason:allowSimulationActivityClock ? 'simulation_observed_activity_waiting_for_fresh_or_minimum_age' : 'simulation_observed_activity_live_mode_not_authorized',
        };
      }
    ...
    // HF6/GCA2-P1: provider milestone coverage is incomplete for several live
    // sports families. In SIMULATION only, preserve a conservative observed
    // activity lower bound instead of starving the experiment forever. This is
    // deliberately NOT LIVE authority: it requires repeated fresh exact-trade
    // observations, continuous evidence without a long gap, and must age through
    // the configured minimum-game-time window before it can authorize a simulated
    // Hunter. Official pregame/conflict/final evidence above always wins.
    if (allowSimulationActivityClock && occurrence.coherent && (!occurrenceTimeMs || occurrenceTimeMs > currentNow) && broadLive(quotes)) {
      const freshActivity = freshTradeActivity(quotes, currentNow);
      const priorStart = Number(prior?.simulationActivityStartMs || 0);
      const priorLast = Number(prior?.simulationActivityLastEvidenceMs || 0);
      const gap = priorLast > 0 ? currentNow - priorLast : 0;
      const continuityBroken = priorLast > 0 && gap > GAME_CLOCK_AUTHORITY.simulationActivityMaxGapMs;
      const start = freshActivity
        ? ((!priorStart || continuityBroken) ? currentNow : priorStart)
        : priorStart;
      const observations = freshActivity
        ? ((!priorStart || continuityBroken) ? 1 : Math.max(1, Number(prior?.simulationActivityEvidenceCount || 0) + 1))
        : Math.max(0, Number(prior?.simulationActivityEvidenceCount || 0));
      const lastEvidence = freshActivity ? currentNow : (priorLast || null);
      const minimumAge = Math.max(0, Number(minimumElapsedMs) || 0);
      const observedAge = start > 0 ? Math.max(0, currentNow - start) : 0;
      const aged = start > 0 && observedAge >= minimumAge && observations >= GAME_CLOCK_AUTHORITY.simulationActivityMinimumObservations;
      if (freshActivity && aged) {
        return makeConfirmed(event, currentNow, start, 'simulation_observed_activity', {
          ...milestoneMeta,
          occurrenceTimeMs,
          ...
          authorizationReason:'simulation_observed_activity_aged_plus_fresh_exact_trade',
          authorizationSource:'simulation_observed_activity_fresh_exact_trade',
          reason:'simulation_observed_activity_authorized',
          simulationActivityStartMs:start,
          simulationActivityLastEvidenceMs:lastEvidence,
          simulationActivityEvidenceCount:observations,
          evidence:{liveData:liveClass,liveError,statsError,milestoneReason:milestoneSelection?.reason||null,milestoneError:milestoneSelection?.error||null,simulationOnly:true,freshTradeActivity:true,observedAgeMs:observedAge,minimumElapsedMs:minimumAge,continuityBroken},
        });
      }
      return makeUnknown(event, currentNow, freshActivity ? 'simulation_observed_activity_aging' : 'simulation_observed_activity_waiting_for_fresh_trade', {
        ...milestoneMeta,
        occurrenceTimeMs,
        simulationActivityStartMs:start || null,
        simulationActivityLastEvidenceMs:lastEvidence,
        simulationActivityEvidenceCount:observations,
        evidence:{liveData:liveClass,liveError,statsError,milestoneReason:milestoneSelection?.reason||null,milestoneError:milestoneSelection?.error||null,simulationOnly:true,freshTradeActivity:freshActivity,observedAgeMs:observedAge,minimumElapsedMs:minimumAge,continuityBroken},
      });
    }
  ...
  async resolveBatch(quotes = [], priorStates = new Map(), now = this.now(), { gameStatsEventTickers = new Set(), allowSimulationActivityClock = false, minimumElapsedMs = 0 } = {}) {
    ...
    const rows = await concurrentMap(events, GAME_CLOCK_AUTHORITY.maxConcurrency, async ([eventTicker, eventQuotes]) => {
      ...
      const state = await this.resolveEvent({ eventTicker, quotes: eventQuotes, priorState: prior, now, allowGameStats, allowSimulationActivityClock, minimumElapsedMs });
