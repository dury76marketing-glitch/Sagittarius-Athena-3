...
export const LEARNING_RUNTIME_LIMITS = Object.freeze({
  crashStates:768,
  crashSummaryStates:100,
  marketDropKeys:4096,
  durationRecorded:4096,
  completedProfitIds:1000,
});
...
function rememberBounded(set,key,limit){
  ...
  set.add(value);
  while(set.size>limit){const oldest=set.values().next().value;if(oldest==null)break;set.delete(oldest);}
}
...
export const CRASH_INTELLIGENCE = Object.freeze({
  version:'CI1',
  persistenceIntervalMs:30_000,
  persistenceRevision:'CI1-P2-KEYED-COALESCED',
  maximumPersistenceConcurrency:1,
  maximumPendingTickers:2048,
  maximumPendingEpisodesPerTicker:64,
...
export const CRASH_RESEARCH_PROFILE = Object.freeze({
  minCrashCents:15,minReboundCents:7,minReclaimRate:0.40,stableObservations:3,upwardTicks:2,episodeResetRate:0.80,
  strategicAuthority:false,role:'feature_and_reference_episode_detection_only',
});

export function crashSettings(_settings = {}) {
  // R51: these thresholds are internal observation geometry only. They can
  // create research/Dragon context for Bolt/Athena, but they are not operator
  // Attack settings and can never veto an Athena FIRE command.
  return { ...CRASH_RESEARCH_PROFILE };
}
...
export function advanceCrashState(priorState, q = {}, settings = {}, now = Date.now()) {
  ...
  // R45 Mega Wave: Golden Dragon is retired from active runtime. Preserve
  // legacy fields/readers for historical audit, but never produce new Golden
  // candidates unless an explicitly legacy settings object requests it. The
  // canonical R45 settings schema has no goldenDragonEnabled key, so normal
  // runtime is deterministically false.
  const goldenRuntimeEnabled = false; // retired runtime invariant; legacy settings cannot reactivate Golden production
  const bid = crashNumber(q.yesBid, 0);
  ...
  if (!distinct) {
    // R59/CI1-F1: a fresh observation can legitimately confirm an unchanged
    // price. Structural counters must not advance, but causal freshness must.
    // R58 returned the old timestamp unchanged here, so A8 could reject a
    // market as ci1_state_stale while the exact quote itself was only
    // milliseconds old. Preserve phase/counters/pending signals verbatim and
    // refresh observation time only. Duplicate observations remain
    // `distinct:false`, so they do not schedule periodic CI1 persistence.
    const refreshed={...prior,lastObservationAtMs:now,updatedAtMs:now};
    return { state:refreshed, transition:'OBSERVATION_REFRESHED', distinct:false, newLow:false, episode:null };
  }
  ...
  if (prior.phase === 'NORMAL') {
    ...
    let goldenPendingSignal = goldenRuntimeEnabled
      ? (prior.goldenPendingSignal
      ...
      : null;
    if (goldenRuntimeEnabled && goldenPendingSignal?.episodeId) {
  ...
  let goldenEntryReady=false;
  let goldenReboundConfirmedAtMs=null;
  if(goldenRuntimeEnabled){
    ...
    goldenEntryReady=goldenStructure.ok;
    goldenReboundConfirmedAtMs=goldenEntryReady ? (priorGoldenReady && legacyGoldenClockValid ? legacyGoldenClock : now) : null;
  }
  ...
  if (prior.phase === 'REBOUND_CONFIRMED' && reclaimRate + 1e-12 >= cfg.episodeResetRate) {
    ...
    const next = {
      ...
      goldenPendingSignal:goldenRuntimeEnabled ? crashSignalSnapshot(completed) : null,
      updatedAtMs:now,
    };
...
export class LearningEngine {
  constructor(db, systemName) {
    ...
    this.crashLastScheduledMs = new Map();
    // R51-HF2/CI1-P2: quote storms must never retain one async PostgreSQL
    // promise per quote. Persistence is keyed by ticker, latest-state-wins,
    // with distinct episode IDs retained inside that bounded ticker job.
    this.crashPersistPending = new Map();
    this.crashPersistActive = new Set();
    this.crashPersistWorkerPromise = null;
    this.crashPersistRetryTimer = null;
    this.crashPersistTotalScheduled = 0;
    this.crashPersistTotalCoalesced = 0;
    this.crashPersistTotalCompleted = 0;
    this.crashPersistTotalFailed = 0;
    this.crashPersistTotalDropped = 0;
    this.crashPersistMaxObservedPending = 0;
    this.crashPersistLastError = null;
    this.goldenDragonHistory = [];
    this.profitStates = new Map();
    this.profitMeta = new Map();
    this.profitLastPersistMs = new Map();
    this.completedProfitIds = new Set();
  ...
  async init() {
    ...
    // R59/RGM4: Golden Dragon has been retired from active runtime since R45,
    // and no production caller consumes goldenDragonSurvivalProfile(). Loading
    // up to 5,000 historical crash rows into every Railway process therefore
    // spent memory/startup DB work with zero trading authority. Keep the legacy
    // compatibility API but leave its in-process corpus empty; the durable
    // historical rows remain untouched in PostgreSQL and Athena uses its own
    // dedicated bounded training readers.
    this.goldenDragonHistory = [];
    if (typeof this.db.crashMarketStates === 'function') {
      const rows = await this.db.crashMarketStates(this.systemName,{limit:LEARNING_RUNTIME_LIMITS.crashStates}).catch(() => []);
      for (const row of rows || []) {
        ...
        const persistedAt=Number(row?.updated_at_ms || state.updatedAtMs || 0);
        this.crashLastPersistMs.set(ticker,persistedAt);
        this.crashLastScheduledMs.set(ticker,persistedAt);
      }
      this.pruneCrashRuntime();
    }
    ...
    if (typeof this.db.completedProfitEpisodeIds === 'function') {
      const ids=await this.db.completedProfitEpisodeIds(this.systemName,{limit:LEARNING_RUNTIME_LIMITS.completedProfitIds}).catch(()=>[]);
      for(const id of [...(ids||[])].reverse())rememberBounded(this.completedProfitIds,id,LEARNING_RUNTIME_LIMITS.completedProfitIds);
    }
  ...
  resourceSnapshot() {
    return {
      crashStates:this.crashStates.size,
      crashPersistClocks:this.crashLastPersistMs.size,
      crashPersistence:{
        revision:CRASH_INTELLIGENCE.persistenceRevision,
        maxConcurrency:CRASH_INTELLIGENCE.maximumPersistenceConcurrency,
        maximumPendingTickers:CRASH_INTELLIGENCE.maximumPendingTickers,
        active:this.crashPersistActive.size,
        pending:this.crashPersistPending.size,
        totalScheduled:this.crashPersistTotalScheduled,
        totalCoalesced:this.crashPersistTotalCoalesced,
        totalCompleted:this.crashPersistTotalCompleted,
        totalFailed:this.crashPersistTotalFailed,
        totalDropped:this.crashPersistTotalDropped,
        maxObservedPending:this.crashPersistMaxObservedPending,
        lastError:this.crashPersistLastError,
      },
      crashStateLimit:LEARNING_RUNTIME_LIMITS.crashStates,
      historicalCrashEpisodes:this.goldenDragonHistory.length,
      profitStates:this.profitStates.size,
      profitMetadata:this.profitMeta.size,
      completedProfitRefs:this.completedProfitIds.size,
      completedProfitRefLimit:LEARNING_RUNTIME_LIMITS.completedProfitIds,
      profitProfiles:this.profitProfilesCache.size,
      marketDropKeys:this.marketDropKeys.size,
      marketDropKeyLimit:LEARNING_RUNTIME_LIMITS.marketDropKeys,
      durationRecorded:this.durationRecorded.size,
      durationRecordedLimit:LEARNING_RUNTIME_LIMITS.durationRecorded,
    };
  }

  pruneCrashRuntime({limit=LEARNING_RUNTIME_LIMITS.crashStates,protectedTickers=[]}={}) {
    const target=Math.max(128,Math.floor(Number(limit)||LEARNING_RUNTIME_LIMITS.crashStates));
    if(this.crashStates.size<=target)return 0;
    const protectedSet=new Set([...(protectedTickers||[])].map(String));
    for(const ticker of this.crashPersistActive)protectedSet.add(String(ticker));
    for(const ticker of this.crashPersistPending.keys())protectedSet.add(String(ticker));
    const removable=[...this.crashStates.entries()]
      .filter(([ticker,state])=>{
        if(protectedSet.has(String(ticker)))return false;
        const phase=String(state?.phase||'NORMAL');
        if(state?.entryReady===true||state?.pendingEntrySignal?.episodeId||state?.goldenPendingSignal?.episodeId)return false;
        return phase==='FINAL'||phase==='NORMAL';
      })
      .sort((a,b)=>n(a[1]?.lastObservationAtMs,a[1]?.updatedAtMs)-n(b[1]?.lastObservationAtMs,b[1]?.updatedAtMs));
    let removed=0;
    for(const [ticker] of removable){
      if(this.crashStates.size<=target)break;
      this.crashStates.delete(ticker);
      this.crashLastPersistMs.delete(ticker);
      this.crashLastScheduledMs.delete(ticker);
      removed++;
    }
    return removed;
  }

  compactForMemoryPressure({limit=512,protectedTickers=[]}={}) {
    const before=this.crashStates.size;
    const removed=this.pruneCrashRuntime({limit,protectedTickers});
    return {before,after:this.crashStates.size,removed,limit:Math.max(128,Math.floor(Number(limit)||512))};
  }

  resetCrashRuntime() {
    this.crashStates.clear();
    this.crashLastPersistMs.clear();
    this.crashLastScheduledMs.clear();
  }
  ...
  crashLearningSummary() {
    // R54/RGM3: aggregate over live references and deep-clone only the bounded
    // display tail. The prior implementation cloned every CI1 state every SSE
    // refresh, creating large transient allocation waves with no trade authority.
    const states=[...this.crashStates.values()];
    let marketsWithCrash=0,totalEpisodes=0,multipleCrashMarkets=0,activeCrashes=0,reboundConfirmed=0,finalized=0;
    for(const x of states){
      const episodes=Number(x?.episodeCount||0);
      if(episodes>0)marketsWithCrash++;
      totalEpisodes+=episodes;
      if(episodes>1)multipleCrashMarkets++;
      if(x?.phase==='CRASHING')activeCrashes++;
      if(x?.phase==='REBOUND_CONFIRMED'&&x?.entryReady)reboundConfirmed++;
      if(x?.phase==='FINAL')finalized++;
    }
    const recent=[...states]
      .sort((a,b)=>Number(b?.updatedAtMs||0)-Number(a?.updatedAtMs||0))
      .slice(0,LEARNING_RUNTIME_LIMITS.crashSummaryStates)
      .map((x)=>structuredClone(x));
    return {
      version:CRASH_INTELLIGENCE.version,trackedMarkets:states.length,marketsWithCrash,totalEpisodes,multipleCrashMarkets,
      activeCrashes,reboundConfirmed,finalized,states:recent,statesIncluded:recent.length,statesOmitted:Math.max(0,states.length-recent.length),
    };
  }

  queueCrashPersistence(state, episode = null, { important=false, finalResult=null } = {}) {
    const ticker=String(state?.ticker||'');
    if(!ticker)return false;
    const now=Number(state?.updatedAtMs||Date.now());
    let job=this.crashPersistPending.get(ticker)||null;
    if(!job&&this.crashPersistPending.size>=CRASH_INTELLIGENCE.maximumPendingTickers){
      // Shed only telemetry work, never trading/protection. Prefer evicting the
      // oldest periodic state snapshot; important/final transitions displace a
      // periodic job if the defensive hard cap is ever reached.
      const candidates=[...this.crashPersistPending.entries()]
        .filter(([,x])=>x?.important!==true&&!x?.finalResult)
        .sort((a,b)=>Number(a[1]?.scheduledAtMs||0)-Number(b[1]?.scheduledAtMs||0));
      const victim=candidates[0]?.[0]||null;
      if(victim){this.crashPersistPending.delete(victim);this.crashPersistTotalDropped++;}
      else if(!important&&!finalResult){this.crashPersistTotalDropped++;return false;}
      else{
        const oldest=[...this.crashPersistPending.entries()].sort((a,b)=>Number(a[1]?.scheduledAtMs||0)-Number(b[1]?.scheduledAtMs||0))[0]?.[0];
        if(oldest){this.crashPersistPending.delete(oldest);this.crashPersistTotalDropped++;}
      }
    }
    if(job)this.crashPersistTotalCoalesced++;
    else job={ticker,state:null,episodes:new Map(),important:false,finalResult:null,scheduledAtMs:now};
    job.state=structuredClone(state);
    job.important=job.important||important===true;
    job.finalResult=finalResult||job.finalResult||null;
    job.scheduledAtMs=now;
    if(episode?.id){
      job.episodes.set(String(episode.id),structuredClone(episode));
      while(job.episodes.size>CRASH_INTELLIGENCE.maximumPendingEpisodesPerTicker){
        const first=job.episodes.keys().next().value;
        if(first==null)break;
        job.episodes.delete(first);this.crashPersistTotalDropped++;
      }
    }
    this.crashPersistPending.set(ticker,job);
    this.crashLastScheduledMs.set(ticker,now);
    this.crashPersistTotalScheduled++;
    this.crashPersistMaxObservedPending=Math.max(this.crashPersistMaxObservedPending,this.crashPersistPending.size);
    this.startCrashPersistWorker();
    return true;
  }

  async persistCrashJob(job) {
    const state=job?.state;
    if(!state?.ticker)return true;
    const now=Number(state.updatedAtMs||Date.now());
    if(typeof this.db.upsertCrashMarketState==='function'){
    ...
    if(typeof this.db.upsertCrashEpisode==='function'){
      for(const episode of job.episodes?.values?.()||[]){
        await this.db.upsertCrashEpisode({...episode,systemName:this.systemName});
      }
    }
    if(job.finalResult&&typeof this.db.finalizeCrashEpisodes==='function'){
      await this.db.finalizeCrashEpisodes(this.systemName,state.ticker,job.finalResult,now);
    }
  ...
  startCrashPersistWorker() {
    if(this.crashPersistWorkerPromise||!this.crashPersistPending.size)return this.crashPersistWorkerPromise;
    if(this.crashPersistRetryTimer)return null;
    this.crashPersistWorkerPromise=(async()=>{
      let failed=false;
      while(this.crashPersistPending.size){
        const [ticker,job]=this.crashPersistPending.entries().next().value;
        this.crashPersistPending.delete(ticker);
        this.crashPersistActive.add(ticker);
        try{
          await this.persistCrashJob(job);
          this.crashPersistTotalCompleted++;
        }catch(error){
          failed=true;this.crashPersistTotalFailed++;this.crashPersistLastError=String(error?.message||error);
          // Merge the failed snapshot back with any newer rerun that arrived
          // while the write was active. The latest state wins, all distinct
          // episode IDs survive, and no quote-level Promise is retained.
          const newer=this.crashPersistPending.get(ticker)||null;
          const retry=newer||{...job,episodes:new Map(job.episodes||[])};
          if(newer){
            for(const [id,ep] of job.episodes||[])if(!newer.episodes.has(id))newer.episodes.set(id,ep);
            newer.important=newer.important||job.important;
            newer.finalResult=newer.finalResult||job.finalResult;
          }else this.crashPersistPending.set(ticker,retry);
        }finally{this.crashPersistActive.delete(ticker);}
        if(failed)break;
      }
    })().finally(()=>{
      this.crashPersistWorkerPromise=null;
      if(this.crashPersistPending.size){
        if(this.crashPersistLastError){
          if(!this.crashPersistRetryTimer){
            this.crashPersistRetryTimer=setTimeout(()=>{this.crashPersistRetryTimer=null;this.startCrashPersistWorker();},CRASH_INTELLIGENCE.persistenceFailureRetryMs);
            this.crashPersistRetryTimer.unref?.();
          }
        }else this.startCrashPersistWorker();
      }
    });
    return this.crashPersistWorkerPromise;
  }

  async flushCrashPersistence(timeoutMs=2_000) {
    if(this.crashPersistRetryTimer){clearTimeout(this.crashPersistRetryTimer);this.crashPersistRetryTimer=null;}
    const deadline=Date.now()+Math.max(0,Number(timeoutMs)||0);
    while((this.crashPersistPending.size||this.crashPersistWorkerPromise)&&Date.now()<deadline){
      this.startCrashPersistWorker();
      const worker=this.crashPersistWorkerPromise;
      if(!worker)break;
      const remaining=Math.max(0,deadline-Date.now());
      let timer=null;
      try{await Promise.race([worker,new Promise((resolve)=>{timer=setTimeout(resolve,remaining);})]);}
      finally{if(timer)clearTimeout(timer);}
      if(this.crashPersistLastError)break;
    }
    return this.crashPersistPending.size===0&&!this.crashPersistWorkerPromise;
  }

  persistCrashState(state, episode = null) {
    // Compatibility surface only. All CI1 persistence, including any future
    // caller of this legacy helper, must traverse the keyed bounded worker.
    return this.queueCrashPersistence(state,episode,{important:true});
  }
  ...
  observeCrashQuote(q, settings, now = Date.now()) {
    ...
    const lastPersist = Math.max(Number(this.crashLastPersistMs.get(ticker) || 0),Number(this.crashLastScheduledMs.get(ticker)||0));
    ...
    if ((important || due) && (typeof this.db.upsertCrashMarketState === 'function' || typeof this.db.upsertCrashEpisode === 'function')) {
      this.queueCrashPersistence(state,result.episode,{important,finalResult:result.transition==='FINAL'?state.finalResult:null});
    }
  ...
  async learnMarketDrops(trackers, quotes) {
    ...
    for (const t of trackers) {
      ...
      const existing = typeof this.db.recoveryObservationByOriginalEntryId==='function'
        ? await this.db.recoveryObservationByOriginalEntryId(this.systemName,key)
        : (await this.db.recoveryObservations(this.systemName, { limit:5000 })).find((x) => x.original_entry_id === key);
      if (existing) { rememberBounded(this.marketDropKeys,key,LEARNING_RUNTIME_LIMITS.marketDropKeys); continue; }
      ...
      rememberBounded(this.marketDropKeys,key,LEARNING_RUNTIME_LIMITS.marketDropKeys);
    }
  }
  ...
  profitLearningState(entryId){const id=String(entryId||'');const s=this.profitStates.get(id);if(s)return structuredClone(s);return this.completedProfitIds.has(id)?{version:PROFIT_LEARNING_INTELLIGENCE.version,phase:'COMPLETE',trackingComplete:true,resident:false}:null;}
  ...
  async finalizeProfitLearning(entry,{finalResult=null,terminalNetCents=null,reason='research_window_complete'}={}){
    ...
    await this.rebuildProfitProfiles().catch(()=>{});
    rememberBounded(this.completedProfitIds,id,LEARNING_RUNTIME_LIMITS.completedProfitIds);
    this.profitStates.delete(id);this.profitMeta.delete(id);this.profitLastPersistMs.delete(id);
    return structuredClone(state);
  }
  ...
  profitLearningSummary(){
    ...
    const residentComplete=states.filter(x=>x.trackingComplete).length;
    return {version:PROFIT_LEARNING_INTELLIGENCE.version,tracked:states.length+this.completedProfitIds.size,complete:residentComplete+this.completedProfitIds.size,postExitTracking:states.filter(x=>x.phase==='POST_EXIT_TRACKING').length,active:states.filter(x=>!x.trackingComplete&&x.phase!=='POST_EXIT_TRACKING').length,residentStates:states.length,completedReferenceCache:this.completedProfitIds.size};
  }
  ...
  async learnDurations(quotes) {
    ...
    for (const q of quotes.values()) {
      ...
      if(typeof this.db.recordSportDurationOnce==='function')await this.db.recordSportDurationOnce(this.systemName,q.ticker,prefix,d);
      else await this.db.recordSportDuration(prefix, d);
      rememberBounded(this.durationRecorded,q.ticker,LEARNING_RUNTIME_LIMITS.durationRecorded);
    }
  }
}
