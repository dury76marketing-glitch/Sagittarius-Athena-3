...
function scores({state,context,observation,newPeak}){
  ...
  const exit= n(observation.executableNetCents)>=-1e-9 && (normalExit||capitalDefense||crashExit);
  ...
  return {
    ...
    exit,exitReason:normalExit?'structural_deterioration':capitalDefense?'capital_defense':crashExit?'severe_crash_deterioration':null,
