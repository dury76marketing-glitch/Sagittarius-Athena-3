...
test('AE1-HF1 PostgreSQL event persistence serializes saints and combination as JSONB text instead of PostgreSQL array literals',async()=>{
  const {Database}=await import('../src/db.mjs');
  const calls=[];const db=Object.create(Database.prototype);
  db.pool={async query(sql,params=[]){calls.push({sql,params});return{rowCount:1,rows:[]};}};
  const event={id:'AE1:S:T:1',systemName:'S',ticker:'T',eventTicker:'E',status:'CANDIDATE',createdAtMs:10,firstVoteAtMs:1,thirdVoteAtMs:3,expiresAtMs:100,convergenceSpanMs:2,saintCount:3,saints:[{conceptName:'Momentum Hunter'},{conceptName:'Wave Surfer'},{conceptName:'Crash Recovery Hunter'}],combination:['Momentum Hunter','Wave Surfer','Crash Recovery Hunter'],review:null,entryId:null,decidedAtMs:null};
  assert.equal(await db.insertAthenaExclamationEvent(event),true);
  const insert=calls.at(-1).params;
  assert.equal(typeof insert[11],'string');assert.equal(typeof insert[12],'string');
  assert.deepEqual(JSON.parse(insert[11]),event.saints);assert.deepEqual(JSON.parse(insert[12]),event.combination);
  await db.updateAthenaExclamationEvent(event.id,{...event,saintCount:4,saints:[...event.saints,{conceptName:'Lightning Plasma'}],combination:event.combination});
  const update=calls.at(-1).params;
  assert.equal(typeof update[6],'string');assert.equal(typeof update[7],'string');
  assert.equal(JSON.parse(update[6]).length,4);assert.deepEqual(JSON.parse(update[7]),event.combination);
});

test('AE1-HF1 terminal formation is removed from active authority and restart hydration does not reactivate it',async()=>{
  const base=Date.now(),event={id:'AE1:SAGITTARIUS:T:1',systemName:'SAGITTARIUS',ticker:'T',eventTicker:'E',status:'CANDIDATE',createdAtMs:base,firstVoteAtMs:base,thirdVoteAtMs:base+2000,expiresAtMs:base+60_000,convergenceSpanMs:2000,saintCount:3,saints:[{conceptName:'Momentum Hunter'},{conceptName:'Wave Surfer'},{conceptName:'Crash Recovery Hunter'}],combination:['Momentum Hunter','Wave Surfer','Crash Recovery Hunter']};
  const persisted=[];const e=new AthenaExclamationEngine({db:{async updateAthenaExclamationEvent(id,row){persisted.push({id,row:structuredClone(row)});}},getSettings:()=>settings()});
  e.events.set(event.id,structuredClone(event));e.activeEventByTicker.set(event.ticker,event.id);
  assert.equal(e.activeCandidate('T',base+3000)?.id,event.id);
  const marked=await e.markDecision(event.id,{status:'EXECUTED',entryId:'ENTRY-1',decidedAtMs:base+4000});
  assert.equal(marked.persistenceOk,true);assert.equal(e.activeCandidate('T',base+5000),null);assert.equal(e.summary(base+5000).executed,1);
  assert.equal(persisted.at(-1).row.status,'EXECUTED');

  const restarted=new AthenaExclamationEngine({db:{async recentAthenaExclamationVotes(){return[];},async recentAthenaExclamationEvents(){return[{...event,status:'EXECUTED',entryId:'ENTRY-1',decidedAtMs:base+4000}];}},getSettings:()=>settings()});
  await restarted.init(base+5000);assert.equal(restarted.activeCandidate('T',base+5000),null);assert.equal(restarted.summary(base+5000).activeFormations,0);
});

test('AE1-HF1 formation persistence failure is visible in module diagnostics instead of disappearing behind vote counters',async()=>{
  const base=Date.now();
  const e=new AthenaExclamationEngine({db:{async upsertAthenaExclamationVote(){},async insertAthenaExclamationEvent(){throw new Error('invalid input syntax for type json');}},getSettings:()=>settings()});
  await e.recordQualification(vote('Momentum Hunter',base));await e.recordQualification(vote('Wave Surfer',base+1000));
  const third=await e.recordQualification(vote('Crash Recovery Hunter',base+2000));
  assert.equal(third.candidate,null);assert.equal(third.reason,'event_persistence_failed');
  const summary=e.summary(base+2000);assert.equal(summary.formations,0);assert.equal(summary.formationPersistenceFailures,1);assert.equal(summary.lastFailure.reason,'event_persistence_failed');assert.match(summary.lastFailure.message,/invalid input syntax/);
});

test('AE1-HF1 active PostgreSQL query excludes terminal or already-consumed Big Bang events',async()=>{
  const {Database}=await import('../src/db.mjs');let call=null;const db=Object.create(Database.prototype);
  db.pool={async query(sql,params){call={sql,params};return{rows:[]};}};
  assert.equal(await db.activeAthenaExclamationEvent('S','T',123),null);
  assert.match(call.sql,/status in \('CANDIDATE','QUALIFIED'\)/);
  assert.match(call.sql,/concept_name='Athena Exclamation'/);
  assert.match(call.sql,/source_trade_id=e\.id/);
});
