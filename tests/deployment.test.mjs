...
test('R37 Golden Eye is intelligence-only and routes execution through the existing all-profitable manual cashout authority',async()=>{
  ...
  assert.ok(engine.includes("this.manualCashout({ entryIds:goldenEyeAuthorityEntryIds }, 'golden_eye_cashout')"),'Golden Eye legacy execution must use the established supervised cashout path scoped away from current ATHENA-X1 attacks');
  const pg=await readFile(resolve(root,'src/profitGuard.mjs'),'utf8');
...
test('R61 dashboard exposes Scarlet continuation controls and cache-busts the new frontend without old retracement doctrine',async()=>{
  ...
  assert.ok(engine.includes("noBoltNoAttackExceptions:['SCARLET_NEEDLE_POST_PROFIT_CONTINUATION','SAGITTARIUS_JUSTICE_ARROW_POST_SHADOW_WIN','ATHENA_EXCLAMATION_POST_12_JUSTICE_ARROWS','LIGHTNING_PLASMA_GEMINI_PLUS_4C']"));
  assert.ok(html.includes('/styles.css?v=R61MW1'));
  assert.ok(html.includes('/app.js?v=R64X1'));
  ...
  try{
    for(const path of ['/','/app.js?v=R64X1','/styles.css?v=R61MW1']){
    ...
    const js=await (await fetch(`http://127.0.0.1:${port}/app.js?v=R64X1`)).text();assert.ok(js.includes("name:'Scarlet Needle'"));assert.ok(js.includes('scarletNeedleMaxRepeats'));
...
test('R55 Phoenix is event-driven, reference-only, TTL-bounded and cannot contaminate frozen Athena B2-R2 context',async()=>{
  ...
  assert.ok(config.includes("SAGITTARIUS-R65-GEMINI-LIGHTNING-PLASMA-ATHENA-EXCLAMATION-2026-08-31"));
...
test('R60 Athena receives COSMO_GREEN directly; Arayashiki and ATB2 remain non-authority research only',async()=>{
  ...
  assert.ok(engine.includes("authorityChain:'GAME_CLOCK->COSMO_SHADOW->COSMO_GREEN->ATOMIC_THUNDER_BOLT->ATHENA->EXECUTION_ATTACK->ATHENA_X1/AURORA'"));
