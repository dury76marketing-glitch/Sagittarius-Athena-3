...
test('R64 full deterministic chain runs Cosmos GREEN -> Bolt -> Athena FIRE -> real SIM Attack -> ATHENA-X1 activation -> ATHENA-X1 exit',async()=>{
  const s=settings({waveSurferEnabled:false,infinityBreakMinNetPerOriginalContractCents:1}),src=shadow({id:'shadow-E2E',ticker:'E2E',entryPriceCents:54}),db=memoryDb([src]),m=market(quote('E2E',55,56));
  const atomic=new AtomicThunderBoltEngine({market:m,getSettings:()=>s,db,systemName:s.systemName,sourceRelease:RELEASE});const bolt=await atomic.detect(m.getQuote('E2E'),{cosmos:[src]});assert.ok(bolt);assert.equal(bolt.greenTrigger.shadowTradeId,src.id);
  ...
  const refreshGameClock=async(q)=>{const now=Date.now();return{gameClockState:{...q.gameClockState,version:'GCA2',phase:'CONFIRMED',confirmed:true,entryAuthorized:true,evidenceObservedAtMs:now,lastCheckedAtMs:now,authorizationReason:'r64_e2e_fresh'},gameStartTimeMs:q.gameStartTimeMs,liveStatus:'live'};};
  const st=new StrategyEngine({db,kalshi:{},market:m,learning:{},getSettings:()=>s,getLiveReady:()=>false,refreshGameClock,random:()=>0});const real=await st.executeAthenaFire(m.getQuote('E2E'),bolt,decision,{cosmos:[await db.entryById(src.id)]});assert.ok(real);assert.equal(real.conceptName,'Momentum Hunter');assert.equal(real.sourceTradeId,src.id);assert.equal(real.status,'open');assert.equal(real.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(real.entryConfig.athenaExit.activationMinimumNetPerOriginalContractCents,1);assert.equal(real.entryConfig.infinityBreak,undefined);
  const learning={async onHardStop(){},async stopGuardProfile(){return null;},async lossWatchdogProfile(){return null;},profitLearningState(){return null;},async observeProfitOpportunity(){return null;},profitRetentionProfileCached(){return{retentionRatio:.92,specificity:'cold_start',promoted:false,totalObservations:0,oneTickPullbacks:0,oneTickRecoveries:0,continuationRate:.5,collapseRate:.25,confidence:'low'};},crashState(){return null;},async profitRetentionProfile(){return this.profitRetentionProfileCached();},async markProfitExit(){}};const guard=new ProfitGuard({db,kalshi:{},market:m,learning,getSettings:()=>s});
  let out;m.setQuote({yesBid:72,yesAsk:73});out=await guard.protect(await db.entryById(real.id));assert.equal(out.closed===true,false);assert.equal((await db.entryById(real.id)).profitGuardState.activationLatched,true);for(const bid of [70,68,66]){await new Promise(r=>setTimeout(r,2));m.setQuote({yesBid:bid,yesAsk:bid+1});out=await guard.protect(await db.entryById(real.id));if(out.closed)break;}assert.equal(out.closed,true);const closed=await db.entryById(real.id);assert.equal(closed.status,'closed');assert.equal(closed.closeReason,'athena_x1_exit');assert.ok(Number(closed.pnlCents)>0);
...
test('R64 complete Scarlet continuation chain runs profitable real ATHENA-X1 parent -> Scarlet authorization -> deterministic SIM entry -> ATHENA-X1 profit exit',async()=>{
  const s=settings({scarletNeedleEnabled:true,scarletNeedleStakeCents:20_000,scarletNeedleMinEntryCents:70,scarletNeedleMaxEntryCents:89,scarletNeedleMaxRepeats:3,simFillProbability:0,galacticExplosionEnabled:true,infinityBreakMinNetPerOriginalContractCents:1});
  const parent={id:'scarlet-full-parent-r64',systemName:s.systemName,ownerId:s.ownerId,conceptName:'Crash Recovery Hunter',ticker:'SCARFULL',eventTicker:'SCARFULL',marketTitle:'SCARFULL',mode:s.mode,status:'closed',remainingCount:0,entryPriceCents:75,exitPriceCents:84,pnlCents:500,closeReason:'athena_x1_exit',closedAtMs:Date.now()-1000,openedAtMs:Date.now()-60_000,entryConfig:{side:'YES',profitAuthority:ATHENA_EXIT_INTELLIGENCE.version}};
  const db=memoryDb([parent]),m=market(quote('SCARFULL',84,85));const refreshGameClock=async(q)=>{const now=Date.now();return{gameClockState:{...q.gameClockState,version:'GCA2',phase:'CONFIRMED',confirmed:true,entryAuthorized:true,evidenceObservedAtMs:now,lastCheckedAtMs:now,authorizationReason:'r64_complete_scarlet_chain'},gameStartTimeMs:q.gameStartTimeMs,liveStatus:'live'};};
  const st=new StrategyEngine({db,kalshi:{},market:m,learning:{},getSettings:()=>s,getLiveReady:()=>false,refreshGameClock,random:()=>1});const engine=Object.create(SagittariusEngine.prototype);engine.settings=s;engine.db=db;engine.market=m;engine.strategy=st;engine.scarletContinuationInFlight=new Set();engine.scarletContinuationStats=null;engine.invalidateStateSnapshot=()=>{};
  const handoff=await engine.handleScarletContinuation(parent);assert.equal(handoff.status,'OPENED');const scarlet=handoff.entry;assert.ok(scarlet);assert.equal(scarlet.conceptName,'Scarlet Needle');assert.equal(scarlet.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(scarlet.entryConfig.athenaExit.activationMinimumNetPerOriginalContractCents,1);assert.equal(scarlet.entryConfig.infinityBreak,undefined);assert.equal(scarlet.entryConfig.aurora.frozen,true);
  const learning={async onHardStop(){},async stopGuardProfile(){return null;},async lossWatchdogProfile(){return null;},profitLearningState(){return null;},async observeProfitOpportunity(){return null;},profitRetentionProfileCached(){return{retentionRatio:.92,specificity:'cold_start',promoted:false,totalObservations:0,oneTickPullbacks:0,oneTickRecoveries:0,continuationRate:.5,collapseRate:.25,confidence:'low'};},crashState(){return null;},async profitRetentionProfile(){return this.profitRetentionProfileCached();},async markProfitExit(){}};const guard=new ProfitGuard({db,kalshi:{},market:m,learning,getSettings:()=>s});
  let out;m.setQuote({yesBid:98,yesAsk:99});out=await guard.protect(await db.entryById(scarlet.id));assert.equal(out.closed??false,false);assert.equal((await db.entryById(scarlet.id)).profitGuardState.activationLatched,true);for(const bid of [96,94,92]){await new Promise(r=>setTimeout(r,2));m.setQuote({yesBid:bid,yesAsk:bid+1});out=await guard.protect(await db.entryById(scarlet.id));if(out.closed)break;}assert.equal(out.closed,true);const closed=await db.entryById(scarlet.id);assert.equal(closed.status,'closed');assert.equal(closed.closeReason,'athena_x1_exit');assert.ok(closed.pnlCents>0);assert.equal(closed.remainingCount,0);
});
...
test('R63 dashboard exposes Gemini as the shadow universe, Another Dimension as its full virtual Attack, and Justice Arrow as an Execution Attack',async()=>{
  ...
  assert.ok(app.includes("legacy:'Sagittarius Justice Arrow'"));assert.ok(app.includes("legacy:'Gemini'"));assert.equal(html.includes('data-cosmo-filter="Another Dimension"'),false);assert.ok(html.includes('/app.js?v=R64X1'));assert.equal(html.includes('/app.js?v=R62ADJA1'),false);assert.ok(html.includes('GEMINI — ANOTHER DIMENSION TRADES'));assert.ok(html.includes('Virtual +1c Profit Authority'));assert.ok(html.includes('<th>Profit Authority</th>'));assert.ok(html.includes('Universal profit authority for every real Execution Attack'));assert.ok(app.includes('GEMINI / ANOTHER DIMENSION COMPLETED +1c NET WIN'));assert.ok(app.includes("e.entryConfig?.profitAuthority||e.entryConfig?.infinityBreak?.version"));
});
