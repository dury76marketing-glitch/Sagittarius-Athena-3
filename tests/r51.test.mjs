...
test('R51 Crystal Wall keeps only structural recovery eligibility and then executes Athena FIRE',async()=>{const f=strategyFixture();const parent={id:'loss-1',systemName:f.settings.systemName,ownerId:f.settings.ownerId,conceptName:'Momentum Hunter',ticker:f.quote.ticker,eventTicker:f.quote.eventTicker,side:'YES',sport:'Tennis',mode:'SIMULATION',status:'closed',remainingCount:0,entryPriceCents:60,exitPriceCents:40,pnlCents:-2000,closeReason:'hard_stop_loss',closedAtMs:Date.now()-1000};const e=await f.s.executeCrystalWallContinuation(f.quote,parent,{authorizationId:'CRYSTAL-WALL-CONTINUATION:loss-1:1',authorizedAtMs:Date.now()});assert.ok(e);assert.equal(e.entryConfig.model.structuralRole,'FOLLOW_ON_RECOVERY_ONLY');assert.equal(e.entryConfig.crystalWallContinuation.parentEntryId,'loss-1');});
