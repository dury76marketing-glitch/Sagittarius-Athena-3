...
import { ATOMIC_THUNDER_BOLT, ATHENA_COMMANDER, ARAYASHIKI, ATHENA_EXIT_INTELLIGENCE, INFINITY_BREAK, AURORA_EXECUTION, SCARLET_NEEDLE } from '../src/doctrine.mjs';
...
test('R65 canonical Attack controls keep stake/band while Lightning Plasma fixed three-slot count is not editable',()=>{
  ...
  assert.equal(keys.has('lightningPlasmaMaxStrikes'),false);
...
for(const concept of ['Momentum Hunter','Wave Surfer','Crash Recovery Hunter'])test(`R51 Athena FIRE executes ${concept} without a second strategic veto`,async()=>{const settings=r51Settings(concept==='Athena Exclamation'?{athenaExclamationEnabled:true}:{});const f=strategyFixture({settings});const c=command(f.settings,concept,f.quote);const e=await f.s.createHunter(concept,f.quote,c.stakeCents,0,{athenaFireCommand:c});assert.ok(e,concept);assert.equal(e.conceptName,concept);assert.equal(e.entryConfig.athenaFire.selectedAttack,concept);assert.equal(e.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(e.entryConfig.athenaExit.activationMinimumNetPerOriginalContractCents,settings.infinityBreakMinNetPerOriginalContractCents);assert.equal(e.entryConfig.infinityBreak,undefined);assert.equal(e.entryConfig.aurora.damageControlPercent,45);});
...
test('R65 fabricated ordinary Lightning Plasma FIRE is rejected because LP2 requires Gemini +4c lineage',async()=>{const settings=r51Settings({lightningPlasmaFieldStakeCents:21000,lightningPlasmaMaxStrikes:3});const f=strategyFixture({settings});const c=command(settings,'Lightning Plasma',f.quote,{stakeCents:7000,fieldBudgetCents:21000,maxRays:3});assert.equal(validateAthenaFireCommand(c,{concept:'Lightning Plasma',q:f.quote,settings}).ok,true);const e=await f.s.createHunter('Lightning Plasma',f.quote,7000,0,{athenaFireCommand:c});assert.equal(e,null);assert.ok(f.db.audits.some(x=>x.event==='lightning_plasma_execution_blocked'&&x.data?.reason==='lightning_plasma_gemini_authority_required'));});
...
test('R51 UI exposes only operator Attack controls plus Plasma rays and configurable Aurora',async()=>{const html=await readFile(new URL('../public/index.html',import.meta.url),'utf8');const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.ok(html.includes('ATOMIC THUNDER BOLT'));assert.ok(html.includes('Infinity Break'));assert.ok(html.includes('Damage Control'));assert.ok(app.includes('auroraDamageControlPercent'));assert.equal(app.includes('lightningPlasmaMaxStrikes'),false);for(const k of oldPredictiveKeys)assert.equal(app.includes(k),false,k);});
...
test('R64 full SIM chain executes Athena FIRE, activates ATHENA-X1 at the operator profit threshold, then exits on confirmed deterioration and completes learning',async()=>{
  const settings=r51Settings({infinityBreakMinNetPerOriginalContractCents:5,infinityBreakRequiredConfirmations:2,infinityBreakMaximumBookAgeMs:1000,infinityBreakConfirmationWindowMs:3000,auroraDamageControlPercent:45});
  const quote=nowQuote('CHAIN-PROFIT',54,55);const f=strategyFixture({settings,quote});const c=command(settings,'Momentum Hunter',quote,{boltId:'chain-profit-bolt'});
  f.db.episodes.set(c.boltId,{id:c.boltId,systemName:settings.systemName,sourceRelease:'R64-TEST',ticker:quote.ticker,eventTicker:quote.eventTicker,side:'YES',sport:'Tennis',boltAtMs:Date.now()-1000,boltSnapshot:{features:{askCents:55,bidCents:54,gameMinutes:45,cosmoSources:['Pegasus']}},athenaDecision:{decision:'FIRE'},fireCommand:c,attackSelected:'Momentum Hunter',trackingComplete:false});
  const entry=await f.s.createHunter('Momentum Hunter',quote,c.stakeCents,0,{athenaFireCommand:c});assert.ok(entry);assert.equal(entry.status,'open');assert.equal(entry.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(entry.entryConfig.athenaExit.activationMinimumNetPerOriginalContractCents,5);assert.equal(entry.entryConfig.infinityBreak,undefined);
  ...
  const protectAt=async(bid)=>{const ts=Math.max(Date.now(),Number(f.market.quote.updatedAtMs||0)+1);f.market.quote.yesBid=bid;f.market.quote.yesAsk=Math.min(100,bid+1);f.market.quote.updatedAtMs=ts;f.market.history.push({t:ts,bid,ask:Math.min(100,bid+1)});return guard.protect(await f.db.entryById(entry.id));};
  let out=await protectAt(70);assert.equal(out.closed??false,false,'threshold crossing activates X1 and is not an immediate sell');let live=await f.db.entryById(entry.id);assert.equal(live.profitGuardState.version,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(live.profitGuardState.activationLatched,true);assert.equal(live.profitGuardState.activationMinimumNetPerOriginalContractCents,5);
  for(const bid of [68,66,64]){out=await protectAt(bid);if(out.closed)break;}assert.equal(out.closed,true,'confirmed deterioration after activation must authorize a full X1 exit');
  const closed=await f.db.entryById(entry.id);assert.equal(closed.status,'closed');assert.equal(closed.remainingCount,0);assert.equal(closed.closeReason,'athena_x1_exit');assert.ok(closed.pnlCents>0);
  const ep=await f.db.opportunityEpisode(c.boltId);assert.equal(ep.trackingComplete,true);assert.equal(ep.outcomeLabel,'CLEAN_BOLT');assert.equal(ep.outcome.athenaX1,true);assert.equal(ep.outcome.infinityBreak,false);
});
...
test('R58 legacy mixed economics stays a bounded cold-start prior until ATB2+A8 certified evidence exists',()=>{
  const settings=r51Settings({infinityBreakMinNetPerOriginalContractCents:5,momentumHunterEnabled:true,waveSurferEnabled:true,recoveryHunterEnabled:false,crashRecoveryHunterEnabled:false,lightningPlasmaEnabled:false,athenaExclamationEnabled:false});
  const entries=[];
  ...
  for(let i=0;i<8;i++)entries.push({id:`w-${i}`,conceptName:'Wave Surfer',status:'closed',closeReason:'hard_stop_loss',pnlCents:-800,count:100,entryFeeCents:200,maeCents:20,entryPriceCents:50,sourceFeeder:'Pegasus',closedAtMs:200+i,entryConfig:{sport:'Tennis',release:'R51-HIST',infinityBreak:{minimumNetPerOriginalContractCents:5},gameClockAuthority:{elapsedMinutes:45}}});
  ...
  const bolt={score:90,features:{askCents:50,bidCents:49,spreadCents:1,sport:'Tennis',gameMinutes:45,cosmoSources:['Pegasus'],cosmoCount:1,velocity15CentsPerSec:0.2,velocity30CentsPerSec:0.1,momentumRiseCents:2,momentumPullbackCents:0,waveFavorableMoveCents:2,crashDepthCents:2,reboundCents:2,reclaimRate:1,upwardTicks:4,lowerLowCount:0,targetFeasibilityScore:90,eligibleAttacks:[band('Momentum Hunter'),band('Wave Surfer')]}};
  const ranking=rankAthenaAttacks(bolt,settings,memory,{});
  const momentum=ranking.find(x=>x.concept==='Momentum Hunter'),wave=ranking.find(x=>x.concept==='Wave Surfer');
  assert.equal(momentum.certifiedEconomicMature,false);assert.equal(wave.certifiedEconomicMature,false);
  assert.equal(momentum.economicAuthorityMode,'ATB2_A8_CERTIFIED_COLD_START');assert.equal(wave.economicAuthorityMode,'ATB2_A8_CERTIFIED_COLD_START');
  assert.equal(momentum.legacyEconomicMature,true);assert.equal(momentum.legacyEconomicQualified,true);assert.equal(wave.legacyEconomicMature,true);assert.equal(wave.legacyEconomicQualified,false);
  assert.ok(momentum.economicDecisionWeight<=0.10+1e-9);assert.ok(wave.economicDecisionWeight<=0.10+1e-9,'legacy negative history cannot dominate certified cold start');
});
...
test('R65 target-aware Bolt and Athena C2 always exclude Athena Exclamation from ordinary strategic discovery',()=>{
  ...
  const features=atomicThunderBoltFeatures({q,history,settings,fieldContext:{athenaExclamationCandidate:{id:'legacy',saintCount:99}},now});
  assert.equal(features.eligibleAttacks.some(x=>x.concept==='Athena Exclamation'),false);
  assert.equal(atomicThunderBoltDecision(features,settings).detected,false);
  const artificial={score:95,features:{...features,eligibleAttacks:[{concept:'Athena Exclamation',targetFeasible:true,targetFeasibilityScore:90,requiredTargetBidCents:64,requiredGrossMoveCents:9,estimatedEntryFeePerContractCents:2,estimatedExitFeePerContractCents:2}]}};
  assert.equal(rankAthenaAttacks(artificial,settings,null,{fieldContext:{athenaExclamationCandidate:{id:'legacy',saintCount:99}}}).length,0);
});

test('R65 fabricated ordinary Athena Exclamation FIRE fails closed without AE2 twelve-arrow Gemini authority',async()=>{
  const settings=r51Settings({athenaExclamationEnabled:true}),q=nowQuote('AE-FIRE',54,55),f=strategyFixture({settings,quote:q});
  const c=command(settings,'Athena Exclamation',q,{boltId:'ae-fabricated-fire'});
  const e=await f.s.createHunter('Athena Exclamation',q,c.stakeCents,0,{athenaFireCommand:c});
  assert.equal(e,null);assert.equal(f.db.inserted.length,0);assert.ok(f.db.audits.some(x=>x.event==='athena_exclamation_execution_blocked'&&x.data?.reason==='athena_exclamation_cycle_authority_required'));
});
...
test('R58 certified economics has no privileged Attack: mature ATB2+A8 evidence can select each structurally eligible Saint',()=>{
  const settings=r51Settings({infinityBreakMinNetPerOriginalContractCents:5,momentumHunterEnabled:true,waveSurferEnabled:true,recoveryHunterEnabled:true,crashRecoveryHunterEnabled:true,lightningPlasmaEnabled:false,athenaExclamationEnabled:false});
  const concepts=['Momentum Hunter','Wave Surfer','Crash Recovery Hunter','Recovery Hunter'];
  ...
  for(const desired of concepts){
    ...
    const ranking=rankAthenaAttacks({score:80,features:baseFeatures},settings,memory,{recoveryContext:{eligible:true}});
...
test('R61 Scarlet Needle V2 is post-profit continuation only and exposes an operator repeat limit',()=>{
  ...
  assert.equal(SCARLET_NEEDLE.trigger,'POST_PROFIT_ATHENA_X1_CLOSE');
...
test('R61 profitable close continuation reuses the complete Hunter safety chain and freezes lineage, Infinity and a fresh Aurora',async()=>{
  ...
  assert.equal(entry.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(entry.entryConfig.athenaExit.activationMinimumNetPerOriginalContractCents,settings.infinityBreakMinNetPerOriginalContractCents);assert.equal(entry.entryConfig.infinityBreak,undefined);assert.equal(entry.entryConfig.aurora.version,AURORA_EXECUTION.version);assert.equal(entry.entryConfig.aurora.frozen,true);
...
test('R64 immediate post-reset A-to-Z chain: Pegasus shadow GREEN -> Atomic Thunder -> Athena FIRE -> Attack -> ATHENA-X1',async()=>{
  ...
  await resetEngine.resetSimulation();assert.ok(resetEngine.market.histories.get('POST-RESET').length>=4);
  ...
  const shadow=await strategy.createGhost('Pegasus',market.quote,53,market.quote.gameStartTimeMs);assert.ok(shadow);const atomic=new AtomicThunderBoltEngine({db,market,getSettings:()=>settings,systemName:settings.systemName,sourceRelease:'R64'});const bolt=await atomic.detect(market.quote,{cosmos:[shadow]});assert.ok(bolt);const athena=new AthenaCommander({db,getSettings:()=>settings,systemName:settings.systemName,sourceRelease:'R64'});const decision=await athena.decide(bolt,{});assert.equal(decision.decision,'FIRE');const entry=await strategy.executeAthenaFire(market.quote,bolt,decision,{cosmos:[shadow]});assert.ok(entry);assert.equal(entry.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);
  const learning={async onHardStop(){},async stopGuardProfile(){return null;},async lossWatchdogProfile(){return null;},profitLearningState(){return null;},async observeProfitOpportunity(){return null;},profitRetentionProfileCached(){return{retentionRatio:.92,specificity:'cold_start',promoted:false,totalObservations:0,oneTickPullbacks:0,oneTickRecoveries:0,continuationRate:.5,collapseRate:.25,confidence:'low'};},crashState(){return null;},async markProfitExit(){}};const guard=new ProfitGuard({db,kalshi:{},market,learning,getSettings:()=>settings});
  const protectAt=async(bid)=>{const ts=Math.max(Date.now(),Number(market.quote.updatedAtMs||0)+1);market.quote.yesBid=bid;market.quote.yesAsk=Math.min(100,bid+1);market.quote.updatedAtMs=ts;market.history.push({t:ts,bid,ask:Math.min(100,bid+1)});return guard.protect(await db.entryById(entry.id));};
  let out=await protectAt(70);assert.equal(out.closed??false,false);assert.equal((await db.entryById(entry.id)).profitGuardState.activationLatched,true);for(const bid of [68,66,64]){out=await protectAt(bid);if(out.closed)break;}assert.equal(out.closed,true);const closed=await db.entryById(entry.id);assert.equal(closed.closeReason,'athena_x1_exit');assert.ok(closed.pnlCents>0);
});
