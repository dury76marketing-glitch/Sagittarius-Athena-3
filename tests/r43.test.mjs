...
function plasmaAd({id,ticker,eventTicker,anchor=50,observedAtMs=Date.now()-1000,status='open'}){
  return {
    id,systemName:'S',ownerId:'O',conceptName:'Another Dimension',ticker,eventTicker:eventTicker||ticker,marketTitle:ticker,
    mode:'SIMULATION',status,entryPriceCents:anchor,currentPriceCents:anchor,peakPriceCents:anchor,count:1,remainingCount:1,
    openedAtMs:observedAtMs,updatedAtMs:observedAtMs,sourceFeeder:'Gemini',sourceTradeId:`G-${id}`,
    entryConfig:{universe:'Gemini'},
  };
}
function plasmaQuote(ticker,eventTicker,bid=54,ask=55){return{...authorizedQuote(ticker,bid,ask),eventTicker:eventTicker||ticker,yesBid:bid,yesAsk:ask};}

test('R65 LP2 qualification is Gemini/Another-Dimension +4c, band/spread bounded and has no time window',()=>{
  const now=Date.now();const s=settings({lightningPlasmaEnabled:true,lightningPlasmaMinEntryCents:45,lightningPlasmaMaxEntryCents:89,maxSpreadCents:3});
  const ad=plasmaAd({id:'ad',ticker:'D',eventTicker:'E',anchor:50,observedAtMs:now-24*60*60_000});
  assert.equal(lightningPlasmaStrikeQualification(ad,plasmaQuote('D','E',53,54),s,now).reason,'uplift_below_four_cents');
  const exact=lightningPlasmaStrikeQualification(ad,plasmaQuote('D','E',54,55),s,now);assert.equal(exact.ok,true);assert.equal(exact.upliftCents,4);assert.equal(exact.noTimeWindow,true);
  assert.equal(lightningPlasmaStrikeQualification({...ad,conceptName:'Dragon'},plasmaQuote('D','E',54,55),s,now).reason,'another_dimension_reference_required');
  assert.equal(lightningPlasmaStrikeQualification(ad,plasmaQuote('D','E',54,90),s,now).reason,'entry_band');
  assert.equal(lightningPlasmaStrikeQualification(ad,plasmaQuote('D','E',54,58),s,now).reason,'spread');
});

test('R65 LP2 field accepts one, two or many asynchronous markets and caps real strikes at three',()=>{
  const now=Date.now(),s=settings({lightningPlasmaEnabled:true,lightningPlasmaMinEntryCents:20,lightningPlasmaMaxEntryCents:89,maxSpreadCents:3});
  const rows=[
    plasmaAd({id:'a',ticker:'A',eventTicker:'SAME-E',anchor:50,observedAtMs:now-100_000}),
    plasmaAd({id:'b',ticker:'B',eventTicker:'SAME-E',anchor:60,observedAtMs:now-20_000}),
    plasmaAd({id:'c',ticker:'C',eventTicker:'EC',anchor:70,observedAtMs:now-5_000}),
    plasmaAd({id:'d',ticker:'D',eventTicker:'ED',anchor:40,observedAtMs:now-1_000}),
  ];
  const map=new Map([['A',plasmaQuote('A','SAME-E',54,55)],['B',plasmaQuote('B','SAME-E',64,65)],['C',plasmaQuote('C','EC',74,75)],['D',plasmaQuote('D','ED',44,45)]]);
  const one=lightningPlasmaFieldSelection(rows.slice(0,1),map,s,now);assert.equal(one.qualifies,true);assert.equal(one.independentMarketCount,1);assert.equal(one.maxStrikes,3);
  const two=lightningPlasmaFieldSelection(rows.slice(0,2),map,s,now);assert.equal(two.qualifies,true);assert.equal(two.independentMarketCount,2);
  const four=lightningPlasmaFieldSelection(rows,map,s,now);assert.equal(four.qualifies,true);assert.equal(four.independentMarketCount,4);assert.equal(four.maxStrikes,3);assert.equal(four.noTimeWindow,true);
  assert.equal(new Set(four.candidates.map(x=>x.ticker)).size,4,'qualification may see >3 markets; engine consumes only free slots');
});

test('R65 LP2 selects at most one Another Dimension reference per exact ticker and field identity is deterministic',()=>{
  const now=Date.now(),s=settings({lightningPlasmaEnabled:true,lightningPlasmaMinEntryCents:20,lightningPlasmaMaxEntryCents:89,maxSpreadCents:3});
  const old=plasmaAd({id:'old',ticker:'A',anchor:50,observedAtMs:now-20_000});
  const fresh=plasmaAd({id:'fresh',ticker:'A',anchor:50,observedAtMs:now-1_000});
  const b=plasmaAd({id:'b',ticker:'B',anchor:60,observedAtMs:now-2_000});
  const map=new Map([['A',plasmaQuote('A','A',54,55)],['B',plasmaQuote('B','B',64,65)]]);
  const f=lightningPlasmaFieldSelection([old,fresh,b],map,s,now);assert.equal(f.independentMarketCount,2);assert.equal(f.candidates.find(x=>x.ticker==='A').source.id,'fresh');
  assert.equal(lightningPlasmaFieldId([fresh,b]),lightningPlasmaFieldId([b,fresh]));
});

test('R65 LP2 fixes three slots and permanently divides the operator field budget across those slots',async()=>{
  const C=await import('../src/config.mjs');const strategy=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');
  const sanitized=C.sanitizeRuntimeSettings({...C.originalSettings(),lightningPlasmaMaxStrikes:99});
  assert.equal(sanitized.lightningPlasmaMaxStrikes,3);assert.equal(C.CANONICAL_NUMERIC_SETTINGS.includes('lightningPlasmaMaxStrikes'),false);
  assert.equal(LIGHTNING_PLASMA.maxStrikes,3);assert.equal(LIGHTNING_PLASMA.fieldBudgetSharedAcrossStrikes,true);assert.equal(LIGHTNING_PLASMA.oneStrikePerTicker,true);assert.equal(LIGHTNING_PLASMA.noTimeWindow,true);
  assert.ok(strategy.includes('Math.floor(configuredBudget/configuredMaxSlots)'));assert.ok(strategy.includes('fieldBudgetSharedAcrossStrikes:true'));
});
...
test('R65 LP2 is scheduled from Gemini/Another Dimension quote observation, not ordinary Cosmo/Athena discovery',async()=>{
  const engine=await readFile(new URL('../src/engine.mjs',import.meta.url),'utf8');const strategy=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');
  assert.ok(engine.includes('queueLightningPlasmaEvaluation'));
  assert.ok(engine.includes('runLightningPlasmaEvaluation'));
  assert.equal(engine.includes('strategy.evaluateLightningPlasma('),false);
  assert.ok(strategy.includes("conceptName || '') !== 'Another Dimension'"));
  assert.ok(strategy.includes("authoritySource:'GEMINI_ANOTHER_DIMENSION_PLUS_4C_UPLIFT'"));
});
