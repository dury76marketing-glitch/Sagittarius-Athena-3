...
test('a committed LIVE exit remains sticky until owned quantity is flattened',async()=>{
  ...
  const db={updateEntry:async(id,p)=>updates.push({id,p}),entryById:async()=>null,audit:async()=>{}};
  const kalshi={buildClientOrderId:()=> 'sag-exit-retry',placeOrder:async(o)=>{order=o;return{orderId:'o1',fillCount:o.count,averageFillPriceCents:o.priceCents};}};
  ...
  await guard.protect({...base,id:'t1',mode:'LIVE',status:'exit_pending',closeReason:'manual_cashout',remainingCount:10,count:10,peakPriceCents:82,stopPriceCents:70,openedAtMs:Date.now()-1000,updatedAtMs:Date.now()-1000});
...
test('committed LIVE U-PG3 IOC uses the positive-profit floor as its sell limit and never authorizes a lower limit',async()=>{
  const now=Date.now();
  const row={...base,id:'live-upg3',mode:'LIVE',status:'open',entryPriceCents:80,currentPriceCents:90,peakPriceCents:96,stopLossCents:35,count:10,remainingCount:10,pnlCents:0,entryFeeCents:20,stopGuardState:{},profitGuardState:{version:'U-PG3',phase:'UPG3_EXIT_COMMITTED',armedAtMs:now-5000,peakExecutableBidCents:96,dangerLineCents:89,structuralLineCents:85,minimumPositivePriceCents:85,economicBreakEvenPriceCents:84,headroomRequiredCents:11,headroomArmPriceCents:96,lastExecutableBidCents:85,failureConfirmations:1,reclaimConfirmations:0},openedAtMs:now-60000,updatedAtMs:now-1000};
  let submitted=null;
  const db={async updateEntry(_id,p){Object.assign(row,structuredClone(p));},async entryById(){return structuredClone(row);},async audit(){}};
  ...
  const kalshi={buildClientOrderId:()=> 'upg3-live-exit',async placeOrder(o){submitted=o;return{orderId:'o-upg3',fillCount:o.count,averageFillPriceCents:87,feePaidCents:20};}};
...
test('R30 APG1 applies the same executable profit covenant to every real Hunter concept without touching feeders',async()=>{
  for(const conceptName of ['Momentum Hunter','Wave Surfer','Recovery Hunter','Crash Recovery Hunter','Dragon Recovery Hunter','Golden Dragon Hunter']){
...
test('R37 Gate 1 creation-time contract freezes Golden Eye + PLI1 on every new real Hunter and excludes feeders',()=>{
  const cfg={mode:'SIMULATION',goldenEyeEnabled:true,goldenEyeLiveEnabled:false,maxPositions:20,maxEntriesPerTrade:1,hunterCooldownMinutes:700,minGameMinutes:20,simFillProbability:.75,simFeeCents:2};
  for(const conceptName of ['Momentum Hunter','Wave Surfer','Recovery Hunter','Crash Recovery Hunter','Dragon Recovery Hunter','Golden Dragon Hunter']){
    const snap=entryConfigSnapshot(cfg,conceptName);
    assert.equal(snap.profitAuthority,GOLDEN_EYE.version,`${conceptName} did not freeze Golden Eye`);
    assert.equal(snap.profitAuthorityRevision,GOLDEN_EYE.policyRevision);
    assert.equal(snap.profitLearning,'PLI1',`${conceptName} did not freeze PLI1`);
  }
  for(const conceptName of ['Pegasus','Sagittarius','Dragon','Golden Dragon']){
    const snap=entryConfigSnapshot(cfg,conceptName);
    assert.equal(snap.profitAuthority,null);
    assert.equal(snap.profitLearning,null);
  }
});
...
test('R38 Gate 1 creation-time snapshot freezes GE1-R2 only on new real Hunters while legacy X1/PRI1 stay compatible',()=>{
  const cfg={mode:'SIMULATION',goldenEyeEnabled:true,goldenEyeLiveEnabled:false,maxPositions:20,maxEntriesPerTrade:1,hunterCooldownMinutes:700,minGameMinutes:20,simFillProbability:.75,simFeeCents:2};
  for(const conceptName of ['Momentum Hunter','Wave Surfer','Recovery Hunter','Crash Recovery Hunter','Dragon Recovery Hunter','Golden Dragon Hunter']){
    const snap=entryConfigSnapshot(cfg,conceptName);
    assert.equal(snap.profitAuthority,GOLDEN_EYE.version);
    assert.equal(snap.profitAuthorityRevision,GOLDEN_EYE.policyRevision);
    assert.equal(snap.profitLearning,'PLI1');
  }
  for(const conceptName of ['Pegasus','Sagittarius','Dragon','Golden Dragon']){
    const snap=entryConfigSnapshot(cfg,conceptName);
    assert.equal(snap.profitAuthority,null);
    assert.equal(snap.profitAuthorityRevision,null);
  }
});
...
test('R37 Gate 5 every real Hunter crosses ATHENA-B1 exactly once, freezes Golden Eye, holds through per-position ProfitGuard, and can complete a full Golden Eye cashout',async()=>{
  for(const concept of ['Momentum Hunter','Wave Surfer','Recovery Hunter','Crash Recovery Hunter','Dragon Recovery Hunter','Golden Dragon Hunter']){
    ...
    const market={getQuote:()=>runtimeQuote,getBook:()=>({updatedAtMs:bookMs}),quoteAgeMs:()=>0,bookAgeMs:()=>Math.max(0,Date.now()-bookMs),async refreshTicker(){return runtimeQuote;},async ensureFreshBook(){return{updatedAtMs:bookMs};},executableAsk(_t,count){return{filled:count,full:true,avgCents:81,bestCents:81};},executableBid(_t,count,min=0){if(runtimeQuote.yesBid<min)return{filled:0,full:false,avgCents:null,bestCents:runtimeQuote.yesBid};return{filled:count,full:true,avgCents:runtimeQuote.yesBid,bestCents:runtimeQuote.yesBid};}};
    const frozen={version:'ATHENA-B1',brainHash:'frozen-r35-b1',ready:true,allow:true,blocked:false,score:67,probability:.67,classification:'SUPPORTIVE',confidence:'HIGH',reason:'assessment',evidence:[{kind:'sentinel'}],assessedAtMs:Date.now()};
    const athena={assess(){b1Calls++;return structuredClone(frozen);}};
    const st=new StrategyEngine({db,kalshi:{},market,learning:{},athena,getSettings:()=>s,getLiveReady:()=>false,refreshGameClock:async()=>({gameClockState:{...quote.gameClockState,evidenceObservedAtMs:Date.now(),lastCheckedAtMs:Date.now()},gameStartTimeMs:quote.gameStartTimeMs,liveStatus:'live'}),random:()=>0});
    st.hunterEntryPolicy=async()=>true;
    st.prepareEntryExecution=async(q,count)=>({count,averagePriceCents:81,freshMarket:{...q,status:'active',result:'',yesBid:80,yesAsk:81}});
    st.revalidateHunterEntryDoctrine=async()=>({ok:true});
    st.exactTickerExposureClear=async()=>true;
    st.simulationAvailableCashCents=async()=>1_000_000;
    const e=await st.createHunter(concept,quote,8_100,35,{});
    assert.ok(e,`${concept} failed to open in integration harness`);
    assert.equal(b1Calls,1,`${concept} did not cross B1 exactly once`);
    assert.deepEqual(e.entryConfig.athena,frozen,`${concept} B1 result was rewritten by Golden Eye`);
    assert.equal(e.entryConfig.profitAuthority,GOLDEN_EYE.version);
    assert.equal(e.entryConfig.profitAuthorityRevision,GOLDEN_EYE.policyRevision);
    assert.equal(e.entryConfig.profitLearning,'PLI1');
    assert.equal(audits.filter(x=>x.event==='athena_candidate_assessment').length,1);
    assert.ok(st.entryPipelineEvents.some(x=>x.stage==='ATHENA'&&x.status==='PASS'));
    assert.ok(st.entryPipelineEvents.some(x=>x.stage==='OPENED'&&x.status==='PASS'));

    const pgLearning={async onHardStop(){},async stopGuardProfile(){return null;},profitLearningState(){return null;},async observeProfitOpportunity(){return null;},profitRetentionProfileCached(){return{promoted:false,specificity:'cold_start',totalObservations:0,oneTickPullbacks:0,oneTickRecoveries:0,continuationRate:.5,collapseRate:.25};},crashState(){return null;},async markProfitExit(){},async finalizeProfitLearning(){}};
    const guard=new ProfitGuard({db,kalshi:{},market,learning:pgLearning,athena:null,getSettings:()=>s});
    runtimeQuote={...runtimeQuote,yesBid:92,yesAsk:93,updatedAtMs:Date.now(),status:'active',result:''};bookMs=runtimeQuote.updatedAtMs;
    const protectedOut=await guard.protect(structuredClone(rows[0]));
    assert.equal(protectedOut.closed??false,false,`${concept} did not survive the Golden Eye hold stage`);
    assert.equal(rows[0].status,'open');
    assert.equal(guard.getState(rows[0].id).guardState,'GOLDEN_EYE_PORTFOLIO_GUARD');
    assert.equal(guard.getState(rows[0].id).goldenEye,GOLDEN_EYE.version);

    const exit=await guard.manualCashout(structuredClone(rows[0]),runtimeQuote,{reason:'golden_eye_cashout'});
    assert.equal(exit.closed,true,`${concept} did not complete the Golden Eye full-profit exit`);
    assert.equal(exit.filled,rows[0].count,`${concept} did not sell the complete owned quantity`);
    assert.equal(rows[0].status,'closed');
    assert.equal(rows[0].remainingCount,0);
    assert.equal(rows[0].closeReason,'golden_eye_cashout');
  }
});
...
test('R36 HARDENING ATHENA-X1 LIVE preflight requires full remaining depth and submits the full owned count',async()=>{
  ...
  const orders=[];
  h.guard.kalshi={buildClientOrderId:()=> 'x1-full-only',async placeOrder(o){orders.push(structuredClone(o));return{orderId:'x1-full',fillCount:o.count,averageFillPriceCents:o.priceCents,feePaidCents:2*o.count};}};
  const decision={action:'athena_x1_exit',reason:'athena_x1_exit',guardState:'ATHENA_X1_EXIT',athenaExitIntelligence:'ATHENA-X1',peakPriceCents:92,stopPriceCents:45};
  let out=await h.guard.liveExit(h.row(),decision);
  assert.equal(out.closed,false);
  assert.equal(out.skipped,'athena_x1_partial_depth_no_split');
...
test('R37 Golden Eye creation-time authority fails safe to ATHENA-X1 whenever the Golden Eye execution lane is disabled for the active mode',()=>{
  const baseCfg={...originalSettings(),systemName:'SAGITTARIUS',ownerId:'sagittarius-main'};
  let snap=entryConfigSnapshot({...baseCfg,mode:'SIMULATION',goldenEyeEnabled:false,goldenEyeLiveEnabled:false},'Wave Surfer');
  assert.equal(snap.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);
  assert.equal(snap.profitAuthorityRevision,ATHENA_EXIT_INTELLIGENCE.policyRevision);

  snap=entryConfigSnapshot({...baseCfg,mode:'LIVE',goldenEyeEnabled:true,goldenEyeLiveEnabled:false},'Wave Surfer');
  assert.equal(snap.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);
  assert.equal(snap.profitAuthorityRevision,ATHENA_EXIT_INTELLIGENCE.policyRevision);

  snap=entryConfigSnapshot({...baseCfg,mode:'LIVE',goldenEyeEnabled:true,goldenEyeLiveEnabled:true},'Wave Surfer');
  assert.equal(snap.profitAuthority,GOLDEN_EYE.version);
  assert.equal(snap.profitAuthorityRevision,GOLDEN_EYE.policyRevision);
});
